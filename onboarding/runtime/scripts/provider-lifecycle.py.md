...
Provider lifecycle commands may create or mutate disposable provider runtime state under `ar-coordination/providers/`, but they should not mutate indexed code repositories or indexed memory roots except by reading them. CGC start/refresh should fail or be treated as unhealthy when source artifact probes find provider-created files in the code repo. GrepAI start/status should fail or be treated as unhealthy when `.grepai/` appears in any configured indexed memory root. Runtime reconciliation may delete stale provider-owned scaffolding and legacy embedded CGC artifacts under `providers/`, but must preserve durable backend data under `provider-data/` unless an explicit destructive database action is requested. Daemon/server actions must run from a durable host process namespace; bounded retrieval/query commands remain usable from sandboxed harnesses.
...
| Finding | Citations | Source Path |
| --- | --- | --- |
| The script imports shared provider layout, GrepAI PostgreSQL constants, artifact detection, CGC patch checks/apply helpers including the visualizer repo-query and route patches, cleanup, workspace-config, and state helpers from the installed runtime skill tree. | L38-L92 | [provider-lifecycle.py](agents-remember-md/runtime/scripts/provider-lifecycle.py) |
...
| GrepAI settings-backed layout expansion uses configured workspace roots and runtime settings, while backend helpers derive PostgreSQL Docker settings, DSNs, embedder settings, readiness probes, pgvector extension creation, and backend state. | L1797-L1991 | [provider-lifecycle.py](agents-remember-md/runtime/scripts/provider-lifecycle.py) |
| GrepAI backend status/start inspect and run the Docker container, validate the data mount against `provider-data/grepai/postgres/data`, allocate or reuse loopback ports, write image locks/state, and report health. | L1994-L2188 | [provider-lifecycle.py](agents-remember-md/runtime/scripts/provider-lifecycle.py) |
| GrepAI install downloads the pinned release asset into `providers/_bin/grepai`, verifies checksums/version, starts the managed backend when configured, rejects dirty source roots, syncs provider-owned mirror roots, and writes provider-owned workspace config. | L2191-L2290 | [provider-lifecycle.py](agents-remember-md/runtime/scripts/provider-lifecycle.py) |
| GrepAI start parses the native background watcher PID from stdout and writes it into lifecycle state so status/stop can use the managed process even when GrepAI's native workspace status does not report it as running. | L1834-L1836; L2395-L2432 | [provider-lifecycle.py](agents-remember-md/runtime/scripts/provider-lifecycle.py) |
| GrepAI status/start/stop/refresh use the runtime-owned binary, workspace name, provider-owned config/log/state/mirror roots, Docker backend status, recorded PID health, process namespace checks for daemon actions, and source-root `.grepai/` checks instead of running from or writing into durable memory roots. | L2293-L2498 | [provider-lifecycle.py](agents-remember-md/runtime/scripts/provider-lifecycle.py) |
| The aggregate `watchers` lifecycle command reads enabled GrepAI and CGC providers from settings, rejects start/stop/shutdown-all in ephemeral process namespaces, dispatches status/start/stop operations to each provider, and exposes parser entrypoints for GrepAI backend actions and watcher actions. | L2517-L2586; L2634-L2695 | [provider-lifecycle.py](agents-remember-md/runtime/scripts/provider-lifecycle.py) |
