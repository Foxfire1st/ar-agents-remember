...
| Finding | Source Path |
| --- | --- |
| The eight registered markers, each naming its opt-in variable and the test file it decorates. | [pyproject.toml](agents-remember/pyproject.toml) |
| The two credential-free jobs, both calling this script with `--require-passed`. | [integration-gated.yml](agents-remember/.github/workflows/integration-gated.yml) |
| The inventory test: every marker is applied, every marker has a runner entry, `ci-safe` is exactly the two CI paths, and the dry-run node exists. | [test_gated_integration_runner.py](agents-remember/mcp/tests/test_gated_integration_runner.py) |
| The Pi RPC smoke suite this runner installs and drives offline. | [test_pi_rpc_real_smoke.py](agents-remember/mcp/tests/test_pi_rpc_real_smoke.py) |
| The real-MCP class whose planning test the generated settings file serves. | [test_tools.py](agents-remember/mcp/tests/test_tools.py) |
