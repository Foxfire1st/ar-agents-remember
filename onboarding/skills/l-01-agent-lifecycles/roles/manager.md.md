...
| --- | --- | --- |
| One manager owns one canonical master and the complete leaf closeout chain. | "**You drive exactly one master's leaf sequence from dispatch to handover.**" | skills/l-01-agent-lifecycles/roles/manager.md:8-12 |
| Hosted child dispatch uses leaf document, role, and complete brief without retained occupant ids. | "Dispatch is one structural transaction." | skills/l-01-agent-lifecycles/roles/manager.md:41-46 |
| The leaf loop sequences builder, reviewer, exact-packet/adjudication curator intake, closeout, integration, and cleanup duties. | "The per-leaf loop" | skills/l-01-agent-lifecycles/roles/manager.md:55-143 |
| Master exit and handover use durable verdict/packet evidence and structural ownership. | "The master-handover packet" | skills/l-01-agent-lifecycles/roles/manager.md:146-152 |
| Structural parent/child messages are the role's communication path. | "**Messages:**" | skills/l-01-agent-lifecycles/roles/manager.md:171-172 |
| Manager dispatch compiles and preserves the exact per-ID acceptance set through reviewer and curator handoffs. | "Per leaf before dispatch:" | skills/l-01-agent-lifecycles/roles/manager.md:24-29 |
