...
- Regular installs reconcile runtime skills, scripts, provider defaults, and coordinator `AGENTS.md` templates.
- Provider installs are package-default only: the installer requires `runtime/providers/` in the source checkout and copies it into the coordination root, but it does not prune live provider runtime directories.
...
| Runtime installation now requires `runtime/providers`, prunes/copies skills and scripts while ignoring Python bytecode caches, copies provider defaults, installs four `AGENTS.md` templates, and creates user-owned coordinator folders. | L198-L208; L241-L264 | [installer](agents-remember-md/installer/install-runtime.py) |
