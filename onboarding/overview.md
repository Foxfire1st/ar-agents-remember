...
| Runtime asset sync treats root runtime folders as canonical and exposes a check form. | `sync_targets` | scripts/sync-runtime.py:236-249 |
...
## Update History
- 2026-09-19T22:58+02:00 — 260918-TSIP-L7 curator (uncommitted change set on `ar/260918-tsip-l7-ar`, memory worktree base `fd1a024e`): **this overview governs `pyproject.toml`, and this candidate changed it — the declared pair is re-read, not carried.** Both statements of the case-budget pair in this document (`## Development And Certification Policy` and `## Build And Development Reference`) read **2,300 unit and 400 integration** as a current fact and now read **3,000 unit and 600 integration** at `pyproject.toml:263-264`, with the raise attributed to the `260918-TSIP-L7` agreement leaf under a direct developer decision and recorded as the first **policy** raise on this line rather than a measured one; the increments before it are retained in the same sentence as the history they are. This is `T56`/`T69`/`T79`'s class and the reason the pair is stated with its ceiling beside it (`T45`): a prose figure is invisible to every checker, which is why the leaf's own agreement module now derives the population by grep instead. The same raise's other homes are enumerated with exact paths in `notes/reports/260918-TSIP-L7-curator-report.md` rather than repaired out of this leaf's change set. `lastVerifiedCommitHash` is deliberately unchanged: the candidate is uncommitted and the governed closeout owns the real code commit. No other claim in this document was re-read.
