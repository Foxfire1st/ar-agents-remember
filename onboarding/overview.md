...
- C-09 wraps task workflows with worktree lifecycle state and also owns direct closeout for approved current-checkout micro edits; it does not replace W-02, starts external-memory worktrees only from a clean committed memory baseline, does not commit, integrate, or clean up without the relevant explicit approval, runs drift detection after the code commit, refreshes memory, runs `memory_quality_check` before the memory commit, and runs cleanup only after successful integration.
...
| memory quality check     | The MCP closeout gate that combines drift integrity and memory style checks.                                  | It runs after onboarding refresh and before the memory content commit; task-start work still uses drift detection only.                              |
...
Updated 2026-05-24T02:47+02:00 after memory quality became the closeout gate for drift integrity and update-history style checks.
