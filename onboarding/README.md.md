...
## Update History
- 2026-09-16T14:05+02:00 — 260915-CAPS-L12 curator: **two claims corrected and one range re-anchored** for the removal of `internal` memory mode (`CAPS-R12@v1`). The Conventions bullet that told maintainers to keep public language describing memory stored "in either repo-local `ar-memory/` or selected external memory repos" now names the memory repository under `ar-coordination/memory-repos/ar-<repo>/` as the only supported topology and states the removal, so the card no longer teaches a mode the product refuses. The `## Stability` claim was both re-anchored and re-worded: `README.md` changed in this candidate, so `README.md:326-329` (the range the generated repair pinned at 2026-09-06) no longer resolves — the section opens at `## Stability` on line 326 and its body is now the single line 328, so the range reads `README.md:326-328`; and the promised layout is `ar-coordination/` and `memory-repos/ar-<repo>/`, not `ar-coordination/`/`ar-memory/`. The two earlier history entries that name the old range and the old layout are historical records of what was true then and were deliberately left as written. Verification metadata remains closeout-owned: the source is uncommitted, so no stamp was advanced and no commit hash was invented.
