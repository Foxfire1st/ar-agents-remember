...
| The README's Status section pins the project at `2.5.2`, identifies `2.5.0` as the CGC durability/readiness release (FalkorDB host-path persistence, graph-content readiness states, degraded-packet propagation, `indexing` summary list, watcher self-heal, orphan cleanup), identifies `2.5.1` as the tool-reliability release (protocol-pipe hygiene, stall-based wedge detection, GrepAI indexing parity, single runner-image derivation, tool-report compaction with keep-5/7-day retention), identifies `2.5.2` as the patch extending that response-budget discipline to `memory_carryover_plan`/`memory_carryover_apply` (per-decision digests inline, full candidate records in `temp/tool-reports/`), describes `2.3.0` as the harness-native starter package release, identifies `2.3.1` as the patch that corrects the MCP package README shown on PyPI, identifies `2.3.2` as the patch that packages the refreshed runtime skills with the C-09 worktree intent approval gate and integration checkout prerequisite reminder, identifies `2.3.3` as the patch that stops/restarts provider watchers around runtime reinstall runner refreshes and normalizes dotted worktree names for Docker-backed provider setup, identifies `2.4.0` as the backward-compatible release for harness-local starter renderers, Python hook command rendering, and manual placeholder-replacement docs that remove the legacy Claude Code `jq` misconception, identifies `2.4.1` as the patch for runtime asset packaging/source sync plus skipped-provider context-packet provider validation, and identifies `2.4.2` as the patch that packages the consolidated `l-01-session-job-lifecycle` skill without the skipped `lifecycle.md` companion file. | L191-L193 | [README.md](agents-remember/README.md) |
