...
| `SubTaskRow`; `TaskSubTaskRefNode`; `SeriesSubTaskNode`; `linkedLifecycleId` | `SubTaskRow`; `TaskSubTaskRefNode`; `linkedLifecycleId` | dashboard/src/types/projection.ts:627-635; dashboard/src/types/projection.ts:649-649 |
...
| `SubTaskIndex` renders in received order and reads the cross-link as `"linkedLifecycleId" in ref`, so the `→` branch is unreachable for a series. | `SubTaskIndex` | dashboard/src/panels/detail-panel/taskReader.tsx:395-433 |
...
| `Markdown`; `Bullets`; `DecisionList`; `MasterSection` | `DecisionList` | dashboard/src/panels/detail-panel/taskReader.tsx:709-724 |
...
- 2026-08-01T09:58+02:00 — 260731-EFA-L4 curator: corrected two false body claims. `SubTaskIndex` no
  ...
  projection-mirror row cit:([`TaskDocNode`, `SeriesNode`, `SeriesSubTaskNode`], dashboard/src/types/projection.ts:399-414; dashboard/src/types/projection.ts:422-429; dashboard/src/types/projection.ts:506-535),
