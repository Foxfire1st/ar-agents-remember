...
| The `task-state` span carries `aria-label` with no role, inside the React Aria `ListBoxItem` whose `role="option"` names it. | "ListBoxItem," | dashboard/src/panels/lifecycle-list/LifecycleList.tsx:6-6 |
