...
- **One control, two entry surfaces** (cit:(["one control, two surfaces"], dashboard/src/panels/session-cockpit/sessions-view/sessionsViewController.ts:213-213)): the file records the one-control/two-surface design seam.
- **Promotion and announcements** (cit:(["The promotion/drift watcher"], dashboard/src/panels/session-cockpit/sessions-view/sessionsViewController.ts:818-818)): the file names the promotion/drift watcher seam.
- **Attention and cycle wiring** (cit:(["hasUnackedSetAttention(perSession[session.id])", "cycleEffortRequested(data.focusedSessionId, direction);"], dashboard/src/panels/session-cockpit/sessions-view/sessionsViewController.ts:336-336; dashboard/src/panels/session-cockpit/sessions-view/sessionsViewController.ts:1028-1028)): the view invokes `hasUnackedSetAttention` for the rail rollup and `cycleEffortRequested` for focused effort commands.
...
- **R8 floor chip prefers pane truth** (cit:(["The VISIBLE pane's real column count"], dashboard/src/panels/session-cockpit/sessions-view/sessionsViewController.ts:197-197)): the file records the live-pane column-count rule.
- **F1 retire-residual sweep** (cit:(["useEffect(() => startRetireResidualSweep(), []);", "export function startRetireResidualSweep(): () => void {"], dashboard/src/panels/session-cockpit/sessions-view/sessionsViewController.ts:817-817; dashboard/src/data/sessionLifecycle.ts:136-136)): SessionsView mounts `startRetireResidualSweep`, whose subscription is refcounted.
...
- **Live `switchSession`** (cit:(["switchSession: (direction) =>", `switchSession`, `switchSession`], dashboard/src/panels/session-cockpit/sessions-view/sessionsViewController.ts:1017-1017) =>"], dashboard/src/panels/session-cockpit/sessions-view/sessionsViewController.ts:1017-1017)): the live command wiring exposes `switchSession`.
...
- **FailedLaunchBanner for a focused FAILED seat** (cit:(["onLaunchCorrected={(prefill) => state.setLaunch({ open: true, prefill })}", "focused.controlState !== \"failed\""], dashboard/src/panels/session-cockpit/sessions-view/sessionsViewBody.tsx:179-179; dashboard/src/panels/session-cockpit/sessions-view/sessionsViewBody.tsx:173-173)): a failed focused seat conditionally renders `FailedLaunchBanner`.
...
- **F6 cycle** (cit:(["The F6 cycle: rail → stage → inspector → status line, collapsed panels excluded (design §5.3).", "const next = nextRegion(current, direction, available);"], dashboard/src/panels/session-cockpit/sessions-view/sessionsViewController.ts:457-457; dashboard/src/panels/session-cockpit/sessions-view/sessionsViewController.ts:472-472)): the file documents the F6 cycle and `nextRegion` supplies the ordered transition.
- **The ~80-col floor chip** (cit:(["const measureStage = useCallback(() => {", "state.setStageNarrow(stageBelowPtyFloor(refs.stageRef.current?.clientWidth ?? 0));"], dashboard/src/panels/session-cockpit/sessions-view/sessionsViewController.ts:764-765)): `measureStage()` computes `stageNarrow` from `stageBelowPtyFloor`.
...
- **Narrow-width rules** (cit:(["Narrow-width rules: rail behavior is unchanged."], dashboard/src/panels/session-cockpit/sessions-view/sessionsViewController.ts:777-777)): the file records unchanged narrow-width rail behavior.
...
- The component must stay safe as a never-unmounted hidden layer: 0-width measures are ignored;
  `active` is passed from Cockpit.tsx (cit:(["active={view === \"chats\" && !takeover}", "useKeyboardZones({ active, dispatch });"], dashboard/src/cockpit/Cockpit.tsx:775-775; dashboard/src/panels/session-cockpit/sessions-view/sessionsViewController.ts:1057-1057)) into this view, which passes it to `useKeyboardZones`.
...
- All decisions (thresholds, floor, calibration percentages) live in data/sessionLayout.ts
  ...
  (cit:([`masterLabels`; `buildRailModel`; `attentionRollup`; `smartDefaultFocus`], dashboard/src/data/railModel.ts:120-129; dashboard/src/data/railModel.ts:192-235; dashboard/src/data/railModel.ts:313-328; dashboard/src/data/railModel.ts:387-403)).
...
| The view mounts the focus-independent retire-residual sweep. | "useEffect(() => startRetireResidualSweep(), []);" | dashboard/src/panels/session-cockpit/sessions-view/sessionsViewController.ts:817-817 |
| The shell passes the active expression to this view. | "active={view === \"chats\" && !takeover}" | dashboard/src/cockpit/Cockpit.tsx:775-775 |
...
| The rail renderer receiving the once-derived model/rollup as props. | `SessionRail`; `model`; `rollup` | dashboard/src/panels/session-cockpit/SessionRail.tsx:56-57; dashboard/src/panels/session-cockpit/SessionRail.tsx:149-236 |
...
| The pure rail/attention/focus derivations are defined in `railModel.ts`. | `masterLabels`; `buildRailModel`; `attentionRollup`; `smartDefaultFocus` | dashboard/src/data/railModel.ts:120-129; dashboard/src/data/railModel.ts:192-235; dashboard/src/data/railModel.ts:313-328; dashboard/src/data/railModel.ts:387-403 |
...
| The shared poll driver subscription is owned by the shell. | "useEffect(() => startCatalogPollDriver(), []);" | dashboard/src/cockpit/Cockpit.tsx:853-853 |
| The `LaunchFlow` component implementation. | `LaunchFlow` | dashboard/src/panels/session-cockpit/LaunchFlow.tsx:362-423 |
...
| SessionsView reads `focusedLiveTurnWorking` from the active conversation projection. | "const focusedLiveTurnWorking = useActiveConversation((state) => {" | dashboard/src/panels/session-cockpit/sessions-view/sessionsViewController.ts:297-297 |
