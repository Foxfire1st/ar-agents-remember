...
| `EngineProcessEdge`'s documented `kind` and `state` vocabularies — `integration` is in one, `refused` in neither, and there is no `refusedPolarity` field. | `EngineProcessEdge` | mcp/src/agents_remember/observer/projection.py:859-878 |
