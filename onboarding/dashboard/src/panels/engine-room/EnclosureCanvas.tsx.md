...
| `EngineProcessEdge`'s documented `kind` and `state` vocabularies — `integration` is in one, `refused` in neither, and there is no `refusedPolarity` field. | `EngineProcessEdge` | mcp/src/agents_remember/observer/projection.py:820-839 |
...
| Projection types `CommitRefNode`/`ProviderBootNode`/`LandingRefNode`, and `ProviderNode`. | `CommitRefNode`; `ProviderBootNode`; `LandingRefNode`; `ProviderNode` | dashboard/src/types/projection.ts:124-132; dashboard/src/types/projection.ts:243-253; dashboard/src/types/projection.ts:348-353; dashboard/src/types/projection.ts:355-366 |
