...
The two scenarios that model a completed landing awaiting reclamation — `engine-cleanup-pending`
(D5 de-materialise) and `engine-landing-merged` (D4, replay + carryover done) — carry
`nextAction: "retry_cleanup"`. That is the automatic post-integration cleanup vocabulary: cleanup
now runs on the successful integration path, and this fixture action names the retry an operator
reaches for when that automatic step did not complete. The retired literal
`request_cleanup_decision` no longer appears anywhere in the file.
