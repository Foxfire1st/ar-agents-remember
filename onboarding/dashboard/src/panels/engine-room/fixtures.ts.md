...
### Cleanup Vocabulary: Stale Demo Data, Deliberately Left (260831-LOCR-L31)

Two scenarios that model a completed landing awaiting reclamation — `engine-cleanup-pending` (D5
de-materialise, `nextAction` at line 982) and `engine-landing-merged` (D4, replay + carryover done,
line 1155) — still carry `nextAction: "retry_cleanup"`. That literal names a move the server
projection can no longer emit: `retry_cleanup` left `NextOperation` when the `cleanup-pending` phase
began routing `lifecycle_finalize_task` (260831-LOCR-L31), so a live projection of these states now
reports the terminal contract status's own next action instead.
...
meeting `retry_cleanup` in this file knows it is neither a current vocabulary member nor a fixture bug
to re-derive from the server. The retired literal `request_cleanup_decision` no longer appears
