...
| `EngineProcessEdge`'s documented `state` vocabulary, which never listed `refused`. | `EngineProcessEdge` | mcp/src/agents_remember/observer/projection.py:934-953 |
...
| `EngineProcessNode` / `EngineProcessEdge` (the `phase` / `edges` / `landing` / `providers` / `seedFallback` / `memoryMode` it reads; `EngineProcessEdge` no longer declares `refusedPolarity`). | `EngineProcessNode`; `EngineProcessEdge` | dashboard/src/types/projection.ts:223-231; dashboard/src/types/projection.ts:233-274 |
...
- 2026-08-01T15:10+02:00 — 260731-EFA-L4 curator (citation pass): repaired the
  `observer/projection.py` citation after that module was restructured. cit:([`EngineProcessEdge`], mcp/src/agents_remember/observer/projection.py:934-953),
  read there: cit:([`EngineProcessEdge`], mcp/src/agents_remember/observer/projection.py:934-953),
  the nine-state comment directly above cit:([`EngineProcessEdge`], mcp/src/agents_remember/observer/projection.py:934-953), and the class's last field
  cit:([`EngineProcessEdge`], mcp/src/agents_remember/observer/projection.py:934-953) — so the "no `refusedPolarity` field" half of the claim is provable at the range
