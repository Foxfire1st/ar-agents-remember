...
| `EngineProcessEdge` declares no polarity field and never documented a `refused` state. | "class EngineProcessEdge" | mcp/src/agents_remember/observer/projection.py:794-794 |
...
- 2026-08-01T15:10+02:00 — 260731-EFA-L4 curator (citation pass): repaired the two
  ...
  below. `L752-L771` → `L762-L781`; read there: `class EngineProcessEdge` cit:(["class EngineProcessEdge"], mcp/src/agents_remember/observer/projection.py:794-794),
  `model_config = ConfigDict(extra="forbid")` (observer/projection.py L779), the nine-state comment (L778) above
  the nine-state comment above `state: str`, and the last field `detail` cit:(["class EngineProcessEdge"], mcp/src/agents_remember/observer/projection.py:794-794). No body claim changed.
