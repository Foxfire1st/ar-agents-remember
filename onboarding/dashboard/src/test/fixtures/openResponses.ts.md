...
| The route decorator exposes the terminal-open API. | `api_terminal_open` | mcp/src/agents_remember/serving/_app_terminal_routes.py:677-692 |
| The terminal response body is assembled by `_terminal_entry_payload`. | "def _terminal_entry_payload(entry: TerminalCatalogEntry) -> dict[str" | mcp/src/agents_remember/serving/_app_terminal_routes.py:206-206 |
| The shared opener returns the resolved terminal-open response. | "def _open_terminal_response(" | mcp/src/agents_remember/serving/_app_terminal_routes.py:224-224 |
