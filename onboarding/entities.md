...
- 2026-08-01T13:20+02:00 — 260731-EFA-L5 curator. Added the `260731-EFA-L5 Entity Clarifications`
  ...
  **Corrected one stale entry this leaf did not cause.** Supervisor Sweep's Migration Notes carried
  ...
  cit:([`SupervisorSignalCooldownStore`], mcp/src/agents_remember/controlplane/supervisor_signals.py:68-215) and `serving/supervisor.py` calls it once per sweep cit:([`run_supervisor_sweep`], mcp/src/agents_remember/serving/supervisor.py:96-193), returning the folded snapshot every `in_cooldown` check
