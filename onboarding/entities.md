...
| lastUpdated | 2026-09-05T07:01:27Z |
...
`ea35964985f30080488270e71ac81657ac40682b`. Entity fingerprints below resolve only committed
`HEAD:<path>` blobs from that candidate; no working-tree fingerprint or provisional verification
stamp is treated as authority. This is source verification of a rejected implementation candidate, not aggregate acceptance. The unchanged fingerprint rows retain byte-identical evidence; the thirteen changed rows were reviewed against their source deltas before refresh.
...
CCR cumulative source verification: Current closeout and direct-landing admission additionally bind the canonical task-intent identity into the operation candidate and require it to equal the claimed door and any retained journal. Missing or stale intent cannot be filled from a plausible legacy value. This is separate from normalized commit messages, the future add-all code tree, and the configured certification profile. R05 exact-candidate certification admission and finalization are still unconnected to this production owner in the reviewed candidate.
