...
## Update History
- 2026-09-20T13:43:00+02:00 — 260915-KS-L45 curator (entity-evidence intersection, scoped review of two rows on an uncommitted change set on `ar/260915-ks-l45-ar`, base `fb719f89`): this leaf edits `mcp/src/agents_remember/serving/app.py`, and that file is inside the declared evidence sets of exactly two rows — **Seat Landing Archive** and **Seat Retirement** — which is why the drift check reports both as "Source has local unstaged changes not represented in HEAD" and reports three further fingerprint-refresh rows. The app.py change is the reviewer route family's registration now passing **both** collaborator ports (the comparison port and the new entry port), plus its two-line call site; the other ten changed sources are the reviewer's transport, application adapter, models, the knowledge store's two enumerating reads, the ingest CLI's candidate/baseline placement and two dashboard files. **Both intersecting rows were re-read against the current source and retained**, and **no evidence path changed and no fingerprint was hand-advanced**: `git-blob-set-v1` resolves committed `HEAD:<path>` blobs, so the stored values remain exactly reproducible against the base commit while this candidate is uncommitted working-tree WIP, and a value computed now would bake in bytes no commit contains. Neither entity's claim is touched — the change mounts one further read-only route beside the existing one and alters no landing, retention, cleanup or retirement behaviour — so no prose was rewritten to accommodate a change that does not reach either entity's subject. Closeout recomputes the intersecting fingerprint rows against the real code commit; the `reviewedWorkingCandidate`-style accounting for this catalog stays as the L23 entry below recorded it, and **no verification stamp was advanced**.
- 2026-09-19T23:05+02:00 — 260918-TSIP-L11 curator (memory worktree `fd1a024e`, code `7879f5b2`): **repaired the fingerprint table this master's own first sync merge split in two, and re-signed nothing by hand.** `ec85ac30a` (the 2026-09-19 merge of the incoming `260915_knowledge-substrate` memory line) inserted the "merged 2026-09-19: fingerprint cells carry each line's as-of values" HTML comment *between* two halves of one GFM table, with a blank line either side, so `parse_entity_fingerprint_rows` read only the first half and 10 inventory entities read as having no fingerprint row (`entity_inventory_without_fingerprint` ×10, measured on this tree before the repair). The comment is preserved verbatim and moved above the header row, where a table note belongs; the 26 rows are one contiguous block again and `style.document_shape.entity_catalog_alignment` reads 0 findings (`check_onboarding_root` run directly on this worktree). **No fingerprint value was recomputed or hand-edited here** — `git-blob-set-v1` resolves committed `HEAD:<path>` blobs and the governed closeout owns the refresh, exactly as the retained note above requires.
...
- 2026-09-18T14:58+02:00 — 260918-TSIP-L3 curator: **one row dispositioned, no entity row added or removed, and no fingerprint value hand-edited.** The census names this catalog's **Curator Coherence Authority** row as an `entity-evidence-intersection` candidate because that leaf changed `mcp/src/agents_remember/models/lifecycles/curator_coherence.py` — one of the row's six evidence paths. The row's prose was re-read against the change and **retained unannotated**: naming the absent publication fields in a refusal and naming them in the registered description does not alter what this entity *is* (the sole live leaf-scoped selection of exact curator judgments), its canonical source-of-truth list, its relationships, or its confusion boundary. **The `git-blob-set-v1` cell was deliberately not re-signed**, for the reason every entry below already states: the algorithm resolves committed `HEAD:<path>` blobs, the leaf's change is uncommitted working-tree WIP, and a value computed now would bake in the base bytes — the governed closeout recomputes the rows whose evidence paths the landed range changes. One thing this leaf can say exactly about that row that the register could not: the row is **already** behind its committed evidence, recorded on 2026-09-13 as stored `sha256:2725f6c7…` against a resolvable `sha256:ada059b5…` with all six files identical between `HEAD` and the worktree, so its re-derivation is the catalog owner's judgment on the **path set** and not a consequence of this leaf. Four further rows report `drifted` for reasons outside this leaf's change set (`Branch-Gated Cross-Repo Source`, `Closeout Effective Input`, `Task Document`, and the same pre-existing `Curator Coherence Authority` staleness); they were re-read and left as found, and they belong to the master-end catalog reading rather than to a leaf's memory commit. No acceptance claim is made.
