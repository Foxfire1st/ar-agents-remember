...
| Coordination Context                | `git-blob-set-v1` | `sha256:77b89f0a8df61e08ded497af503296e68bf25d20e6dbfb6fef51d50dd70ff99f` | `mcp/src/agents_remember/package_data/runtime/skills/c-08-ar-coordination-context-resolver/SKILL.md`; `mcp/src/agents_remember/kernel/coordination_context_resolver.py`                                                                                                                                                                                                                                                 |
| Path Rule                           | `git-blob-set-v1` | `sha256:cff57b277cfdee05f63dbe4c2066f804a27ff7fbdf0076c958ae1acb45a464a2` | `mcp/src/agents_remember/kernel/coordination_context_resolver.py`; `mcp/src/agents_remember/package_data/runtime/system/defaults/examples/memory-repo/settings.json`; `examples/mcp/settings.example.json`                                                                                                                                                                                                       |
| Memory Quality Control              | `git-blob-set-v1` | `sha256:d87a72999187b1d68dd23ab247c2d76fccfcfcb08e5f7bb47123704133b1eedd` | `mcp/src/agents_remember/package_data/runtime/skills/c-02-memory-quality-control/SKILL.md`; `mcp/src/agents_remember/memory_quality/check.py`; `mcp/src/agents_remember/memory_quality/integrity/check_missing_onboarding.py`; `mcp/src/agents_remember/memory_quality/integrity/onboarding_drift_check/drift.py`; `mcp/src/agents_remember/memory_quality/integrity/onboarding_drift_check/summary.py`; `mcp/src/agents_remember/memory_quality/style/update_history/history_order.py`; `mcp/src/agents_remember/memory_quality/style/update_history/history_order_fix.py` |
...
`worktrees/modules/guidance.py`, never the reverse. `worktrees/status.py` now builds
`WorktreeSummary` instead of returning a dict for `controllers/context_packet.py` to
...
**The dashboard mirror is hand-maintained, and nothing checks it against the server.** Count the
links, because the two-set shorthand for this drops one. The chain is **four nodes and three
links**: `dashboard/src/test/fixtures/wire.ts` →A→ `dashboard/src/types/projection.ts` →B→
`dashboard/src/fixtures/snapshot.json` →C→ `mcp/src/agents_remember/observer/projection.py`.
**Link A** is `tsc -b`: fixture bases are annotated with their mirror type and call-site overrides
go through `Overrides<O, Node>`, with `wireFixtureGuard.test.ts` refusing the one-token opt-outs.
**Link B** is `dashboard/src/test/contract.test.ts`, and it is *not* a one-way containment — three
type-level directions (the mirror declares everything the sample carries; the sample carries
everything the mirror declares; the sample *reaches* every path the mirror declares) plus runtime
vocabulary assertions for the string unions the JSON import widens away. **Link C is held by
nothing.** `snapshot.json` is written by hand to stand in for the server: seven files reference it
and every one is a reader, no script under `mcp/`, `scripts/` or `dashboard/` writes it,
`dashboard/package.json`'s `codegen` script is Panda CSS, and neither dependency set carries a
schema-to-type tool. That establishes **no in-repo generator and no in-repo mechanism keeping the
two sides in step**; it cannot exclude a generator run outside this repository, and no search of
this tree could. Note also that `mirror ⊆ served` — against the sampled payload — **is** enforced,
while `mirror ⊆ server` is not; the two phrases differ by one letter, which is why the links are
named by their files here. A field the server starts sending that neither the snapshot nor the
mirror knows about is invisible to all of it. Nothing here should ever be read as automatic
synchronization.
...
| Context packet    | `worktrees.status.worktree_status_packet` constructs `WorktreeSummary` directly instead of returning a dict for the caller to validate, and omits `nextTool`/`nextArgs`/`nextRequiredArgs` when the producer omitted them rather than substituting `""`/`{}`/`[]`. |
...
- 2026-08-01T10:50+02:00 — 260731-EFA-L4 curator, **three corrections to this register, all
  ...
  (`LIVE_STATES` L42, `TERMINAL_STATES` L48), `LIFECYCLE_STATES` is spread from them (L59), and
  `ACTIVE_STATES = LIVE_STATES` (L72) with no `Exclude<>` anywhere. Replaced with the current fact
