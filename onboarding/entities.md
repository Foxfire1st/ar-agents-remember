...
| Description                  | The helper parses and writes the fenced `json ar-memory-ledger` metadata plus newest-first code-to-memory mappings. In journaled worktree closeout, `closeout_external.py` is the sole external phase owner: after memory refresh it uses the accepted explicit memory message, records mutation intent, and proves or reuses the memory-content commit; it then uses the accepted explicit ledger message to prove or create the mapping commit. These are two sequential Git commits, not an atomic transaction. There is no generated ledger subject or blank/default fallback. Integration, start/carryover, and baseline retain their separate ledger call sites. The durable mapping authority is the committed Git object; mutation evidence and reachability decide worktree closeout recovery. |
| Canonical Source Of Truth    | `mcp/src/agents_remember/kernel/memory_ledger.py` plus service call sites in `mcp/src/agents_remember/worktrees/modules/closeout_external.py`, `mcp/src/agents_remember/worktrees/modules/integrate.py`, and `mcp/src/agents_remember/memory/baseline.py`; `closeout.py` is the coordinator, not the ledger writer. |
...
| Description | Each sprint projection is derived from an exact-current census of canonical task topology, waiting door generations, and active series sources. Its service condition is only `invalid-empty` or `valid-built`; valid members are classified `ready`, `waiting`, or `blocked` and carry deterministic priority, order, and bounded reasons. Canonical mutation invalidates to durable empty first, rebuilds off-side without consulting old rows, and publishes only if the source identity is still exact-current. The projection owns no claim, worker, commit, certification, integration, cancellation, recovery, or terminal evidence. |
| Canonical Source Of Truth | `models/closeout_projection.py` defines strict projection state/member/problem/effect models; `controlplane/closeout_queue_store.py` owns invalid-empty/valid-built persistence; `worktrees/queue/closeout_projection.py`, `closeout_projection_members.py`, and `closeout_projection_publication.py` own exact source census, member computation, and invalidation/rebuild publication. `models/queue/closeout_queue.py` and `worktrees/queue/closeout_queue.py` expose only sprint-scoped status/rebuild plus the short first-ready admission fence. Task/door truth is upstream and journal truth remains separate. |
...
| Parent / Child Relationships | Belongs to one sprint and is rebuilt from current task, door, and series truth. A short task/door publication CAS may require the exact first-ready generation at claim admission; after claim, the door and enclosure-external operation journal own lifecycle evidence. Task authoring remains authoritative and can invalidate/rebuild affected projections without queue permission. |
| Often Confused With | The sprint task document, a closeout-door generation, Judgment/Priority Registers, the operation journal, a generic durable job queue, the external-memory ledger, the landing lane, or the terminal archive. |
| Source References | `mcp/src/agents_remember/controlplane/closeout_queue_store.py`; `mcp/src/agents_remember/models/closeout_projection.py`; `mcp/src/agents_remember/models/queue/closeout_queue.py`; `mcp/src/agents_remember/worktrees/queue/closeout_projection.py`; `mcp/src/agents_remember/worktrees/queue/closeout_projection_members.py`; `mcp/src/agents_remember/worktrees/queue/closeout_projection_publication.py`; `mcp/src/agents_remember/worktrees/queue/closeout_queue.py` |
| Migration Notes | Introduced by 260815-DAG and completed by 260821-CLIVE L3. The transitional selected/in-flight/certified rows, task-document queue veto, blocker/lifecycle commands, and `closeout_queue_lifecycle.py` owner were removed rather than retained behind compatibility readers. Current rebuild never consumes stale rows, and lifecycle evidence survives only at its door/journal owners. |
...
| Canonical Source Of Truth | `models/closeout_input.py` defines the vocabulary; `worktrees/closeout_input.py` owns plan derivation and normalization; `worktrees/integration/closeout_operation_admission.py` owns lease-stable durable admission. |
...
| Description | Each enabled repository leg advances through `pre-mutation`, `mutation-intent`, `reconciled-unchanged`, or `commit-proven` evidence bound to branch/ref, HEAD/tree, reflog fingerprint, index/candidate trees, and worktree status. Intent is journaled before Git; commit proof validates the exact ref/parent/tree transition. After restart, exact unchanged state is distinguished from exact expected output and from ambiguity such as a ref moving away and back. A public retry of unchanged intent preserves attempt one, does not launch implicitly, and remains cancellable; status or reflog observation failure leaves the literal journal and evidence unchanged. Commit-proven evidence derives the compatibility recovery tuple, while exact canonical contract-publication proof retains verified-existing/no-op generations without fabricated Git mutation evidence. |
| Canonical Source Of Truth | `models/lifecycles/mutation_evidence.py`, `worktrees/integration/mutation_evidence.py`, `worktrees/integration/closeout_recovery_projection.py`, and the strict lifecycle operation store. |
