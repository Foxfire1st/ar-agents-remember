...
- First-class service domains such as `kernel`, `providers`, `drift`,
...
- `providers/runners`, `providers/data`, and `providers/logs` are the active
  ...
  runtime roots.
- The copied resolver, provider lifecycle, drift, and worktree code under
  `mcp/src/agents_remember` is a package-local implementation surface. Original
  runtime scripts remain present but are not the MCP execution authority.
