...
| The failure cases that assert a refusal advertises its own reason and remedy. | `test_a_refusal_still_produces_an_explanation_manifest` | mcp/tests/test_role_capsule_compiler.py:893-907 |
...
| The typed failure of a two-leg atomic publish, subclassing both the domain family and `OSError` so existing `except OSError` call sites keep observing it. | `AtomicReplaceError` | mcp/src/agents_remember/errors.py:566-608 |
| The publisher whose two legs raise it, one label per leg, with the destination's state. | `atomic_replace` | mcp/src/agents_remember/kernel/atomic_write.py:80-126 |
...
## Update History
- 2026-09-18T19:20+02:00 — 260915-KS-L23 curator (uncommitted change set on `ar/260915-ks-l23`, base `c5a74a85`): recorded the new `AtomicReplaceError` this change set appends at `566-608` — the typed failure of the two-leg `atomic_replace` publish, carrying `leg` (`replace` / `directory-fsync`) and `destination_state` (`previous-bytes` / `source-absent`) plus a bounded `response_fields()` projection — and stated the two boundary facts a caller depends on: it subclasses **both** `AgentsRememberError` and `OSError` so existing `except OSError` call sites keep observing a failed publish, and its labels describe the destination's *measured* state, not the operation's intent. Also corrected this card's general account of the module's tail: it said the role-capsule family "adds a fourth family at the end of the module", which stopped being true when `TaskProjectionSourceError` (`516-563`) and `MemoryModeUnsupportedError` (`611-659`, already committed in `b281bcd6`) were appended below it; the new paragraph now names `AtomicReplaceError` between them rather than implying the module ends with the capsule family. The append is additive (nothing above `566` moved), so no pre-existing range in this card or in any other card citing `errors.py` needed repair. Recorded one contradiction inside the change itself rather than propagating it: the class docstring lists a third state, `"new-bytes"` ("the rename completed and its name is not yet durable"), which **no raise site produces** — the two raise sites emit `"previous-bytes"` and `"source-absent"` only — so this card names it as declared-but-unreachable instead of repeating it as an observed state. Added two Repo-Internal rows for the class and its only producer. Two pre-existing body citations into this file remain one line long at their end bound (`CapsuleCompilationError` cited `464-506` while the class spans 464-505; `render`/`response_fields` cited `490-496`/`497-506`) — they were left exactly as their leaf wrote them, for the citation pass to re-derive against this uncommitted candidate; verification metadata is unchanged and closeout owns the stamp.
...
- 2026-09-18T05:26:45+00:00: Generated citation repair: `test_a_refusal_still_produces_an_explanation_manifest` repointed to mcp/tests/test_role_capsule_compiler.py:893-907. No content impact: mechanical anchor-range projection bound to citation source snapshot 70078cc4ca208e40e9a66742bdc38893ecfb1757ca7d77a526e6ba2159339959; claim bytes unchanged; generated by ccr-r10@v1.
