...
| Finding | Anchor | Source |
| --- | --- | --- |
| The one line that places the capsule-compiler port on the serving runtime. | `_build_serving_runtime`; `ServingCollaborators.capsule_launch` | mcp/src/agents_remember/serving/app.py:165-251; mcp/src/agents_remember/serving/_app_common.py:455-462 |
...
- 2026-09-15T20:42+02:00 — 260831-LOCR-L17 curator (uncommitted change set on `ar/260831-locr-l17`,
  ...
  current-intent section was added rather than an annotation. `_build_serving_runtime` now resolves
  one `serving_clock` for the sweeper, the runtime and the new
  `TerminalObserverHealthPublisher(observer_root(config), serving_clock)`, and places the publisher
  on `_ServingRuntime.observer_health` — one construction site, so the lifespan's writes and the
  routes' reads cannot land on different roots or lifetimes, and a sim replay substitutes one clock
  everywhere. Recorded the boundaries with it: no second endpoint, alias, history log or fallback
  store, and no catalog, closeout, queue or task-authoring authority derived from health.
  Reference ranges re-derived against the 390-line candidate (`create_app` `230-301` → `253-310`,
  the export row `323-323` → `332-332`) with one new row. Verification metadata remains
  closeout-owned; no stamp advanced.
- 2026-09-15T13:57+02:00 — 260831-LOCR-L02 curator: **No content impact.** `app.py` is not in this
  leaf's change set (`git status --porcelain` = the route, the new test module, and the lane
  manifest), and this card's current-intent sections carry no claim that the terminal-session GET
  route produces catalog state, so no body sentence here is contradicted. **Recorded because three
  historical entries in this card do carry such a claim and are now superseded**, and a reader
  scanning newest-first should not take them for current intent: the 2026-07-07T23:45+02:00 entry
  says "`GET /api/terminal/sessions` returns `liveness_sweeper.refresh()`", the 2026-07-09T14:05+02:00
  entry says the route "returns sweep-cold landed rows", and the 2026-06-26T23:05+02:00 entry says it
  "refreshes stale rows". Each was accurate when written and stays as written, because `Update
  History` is append-only; the current contract lives in the route's own card
  ([_app_terminal_routes.py](_app_terminal_routes.py.md)) and in the serving overview, where the GET
  is a projection of `runtime.catalog.list()` that names no sweeper. Verification metadata stays
  closeout-owned; `lastVerifiedCommitHash` remains pinned to the last commit that verified this card.
- 2026-09-05T06:24:16+00:00: Generated citation repair: "\"TerminalAttachTaskRequest\"," repointed to mcp/src/agents_remember/serving/app.py:332-332. No content impact: mechanical anchor-range projection bound to citation source snapshot ad34c1284f637cc2e60117d5a156ddfdd2236402d2c1332758dd691c2cbef881; claim bytes unchanged; generated by ccr-r10@v1.
...
- 2026-08-01T09:44+02:00 — 260731-EFA-L4 curator: recorded the de-injection and the route
  ...
  name. Corrected cit:(["def _agent_notifier_heartbeat_payload(runtime: _ServingRuntime) -> AgentNotifierHeartbeatPayload:"], mcp/src/agents_remember/serving/_app_lifespan.py:265-265) and cit:(["async def stream_events("], mcp/src/agents_remember/serving/_app_common.py:116-116),
