...
`app._supervisor_heartbeat_payload` used to build that answer as a bare `dict[str, Any]` and
write it straight into an already-dumped, already-validated projection body under
`supervisorHeartbeat` — a key nothing declared. It now returns this model, and
...
`redeliverableInboxCount`, `lastSweepDurationSeconds`) cit:(["def _supervisor_heartbeat_payload(runtime: _ServingRuntime) -> SupervisorHeartbeatPayload:"], mcp/src/agents_remember/serving/_app_lifespan.py:185-185) cit:([`ServedWorkspaceProjection`], mcp/src/agents_remember/serving/served_state.py:47-55).
...
- **Backlog metrics are volatile status.** Pending/redeliverable inbox counts and last sweep
  ...
  reducer truth.
- **The row and the read-side payload stay separate types.** `SupervisorHeartbeatPayload` may
  gain response-time arithmetic (`ageSeconds`, `stale`) that must never be persisted into
  `supervisor-heartbeat.json`, and `SupervisorHeartbeat` must never carry a per-response value.
