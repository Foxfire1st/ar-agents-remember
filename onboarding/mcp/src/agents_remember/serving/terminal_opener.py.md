...
| The catalog row owns durable seat, lineage, resolved knob, free-form, and control metadata. | "class TerminalCatalogEntry" | mcp/src/agents_remember/models/terminal_catalog.py:68-68 |
| The catalog batch holds both the process lock and instance lock across the complete unit of work. | `batch` | mcp/src/agents_remember/serving/terminal_catalog.py:259-291 |
| The dashboard route maps actual-row success/conflict facts without duplicating spawn composition. | "def _open_terminal_response(" | mcp/src/agents_remember/serving/_app_terminal_routes.py:224-224 |
