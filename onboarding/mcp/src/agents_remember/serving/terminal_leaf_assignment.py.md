...
`assign_terminal_session_to_leaf` performs the durable move. It rejects a missing or terminated row as
...
- `leaf-taken` is a no-mutation result. Callers must not update local state or inject context after this
  status.
- Terminated sessions cannot claim leaves. Exited rows follow existing catalog semantics and are not
  treated as active owners.
