...
`read_entry_terminal_evidence` cit:([`read_entry_terminal_evidence`], mcp/src/agents_remember/serving/terminal_evidence.py:148-164) is the per-row entry point: non-harness,
...
`terminal_evidence_sequence` and map it with `latest_terminal_evidence` cit:([`latest_terminal_evidence`], mcp/src/agents_remember/serving/terminal_evidence.py:66-104). The
...
`_read_pi_terminal_evidence` cit:([`_read_pi_terminal_evidence`], mcp/src/agents_remember/serving/terminal_evidence.py:166-194) walks from the persisted `terminal_native_cursor` to
...
`TerminalEvidenceRead` cit:(["class TerminalEvidenceRead:"], mcp/src/agents_remember/serving/terminal_evidence.py:52-63) is the no-loss contract: `projection` plus the optional
...
`interrupted_origin` cit:([`interrupted_origin`], mcp/src/agents_remember/serving/terminal_evidence.py:196-209) attributes an interrupted outcome: `developer` only when the
...
| --- | --- | --- |
| No external/domain document defines this lift; the projectors and control-plane evidence pages are the source of truth. | `read_entry_terminal_evidence` | mcp/src/agents_remember/serving/terminal_evidence.py:148-164 |
...
| Finding | Anchor | Source |
| --- | --- | --- |
| The per-vendor projector registry the lift reuses. | `projector_for` | mcp/src/agents_remember/serving/conversation/projectors/__init__.py:120-121 |
| The bounded evidence-page and native-page read seams. | `read_control_evidence`; `read_control_native_page` | mcp/src/agents_remember/serving/harness_control_client.py:363-383; mcp/src/agents_remember/serving/harness_control_client.py:387-419 |
| The catalog row fields this module reads and the liveness sweep that calls it (read-before-projection). | "class TerminalCatalogEntry:"; `_observe_alive` | mcp/src/agents_remember/serving/terminal_catalog.py:106-220; mcp/src/agents_remember/serving/terminal_liveness.py:343-426 |
| The forcing regression suite for projection, pi paging, origin resolution, and cursor no-loss semantics. | `LatestTerminalEvidenceTests`; `ReadEntryTerminalEvidenceTests`; `OriginResolutionTests` | mcp/tests/test_terminal_evidence_projection.py:180-283; mcp/tests/test_terminal_evidence_projection.py:285-471; mcp/tests/test_terminal_evidence_projection.py:620-642 |
