...
`seat_events.py` emits the observer events (260707-HFX-L8) that feed the watcher/architect
NEEDS-ATTENTION feed for seat retirement, rename, and turn-state transitions. It exists so both the
`session_retire`/`session_rename` MCP tools AND the L5 liveness-sweep turn-state wiring
...
Three functions, each building one `Event` and appending it via `EventStore(observer_root(config))`:

- `log_retire_event(config, entry)` — kind `"seat.retired"`, `ts=entry.retired_at or now_iso()`,
  `trust="observed"`, `actor="system" if entry.retired_by_session is None else "model"` (the
  automation hooks pass `by_session=None`, so an auto-retire is attributed to `"system"`; a manual
...
All three mirror the existing `orchestration_nudge_manager` event-logging pattern
...
| `api_terminal_retire`/`api_terminal_rename` call the same functions from the serving endpoints; `create_app` wires `on_turn_state_change=lambda observation: log_turn_state_change_event(config, observation.entry)` into the liveness sweeper. | `api_terminal_retire`; `api_terminal_rename`; `create_app` | [app.py](app.py) |
| `_auto_retire_completed_seats` calls `log_retire_event` for every auto-retired seat, inside the same best-effort `try/except Exception` guard that wraps the whole retire body (F1 fix). | `_auto_retire_completed_seats` | [../controllers/worktree_tools.py](../controllers/worktree_tools.py) |
