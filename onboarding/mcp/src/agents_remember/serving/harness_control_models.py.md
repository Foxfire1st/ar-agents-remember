...
| The codex adapter's `_sync_pending_snapshot` preserves the parent pending interaction and adds raw `threadId` plus `agentLabel` for non-parent entries. | `_sync_pending_snapshot` | mcp/src/agents_remember/serving/codex_app_server_adapter.py:960-972 |
