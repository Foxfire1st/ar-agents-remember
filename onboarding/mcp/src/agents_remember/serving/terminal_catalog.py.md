...
| Finding | Anchor | Source |
| --- | --- | --- |
| The FastAPI app injects/creates the catalog, refreshes stale rows, rehydrates WebSockets from catalog metadata, persists opener rows, marks terminations, and uses catalog cwd for image uploads. | `create_app` | mcp/src/agents_remember/serving/app.py:226-285 |
