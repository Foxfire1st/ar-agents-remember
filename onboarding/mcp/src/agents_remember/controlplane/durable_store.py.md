...
| Finding | Citations | Source Path |
| --- | --- | --- |
| The `CONTRACT FRONT MATTER` block declaring `ar-durable-store/1.0`, the unconditional per-log serialization, single-owner compaction, the local-POSIX platform constraint and the two read policies. | module front matter | [durable_store.py](agents-remember/mcp/src/agents_remember/controlplane/durable_store.py) |
| The heading that separates the two mechanisms: the lock is unconditional and is what took the loss to zero, while ownership is advisory and opt-in and works structurally rather than at runtime. Also where the module names all three `declare_process_role` call sites, the third being the `--reload` spawn worker. | the `WHAT PREVENTS LOSS, AND WHAT MERELY DOCUMENTS` section | [durable_store.py](agents-remember/mcp/src/agents_remember/controlplane/durable_store.py) |
| The read-policy section: strict for authority because a skipped record could drop an `applied` marker, tolerant for projection because a tick must degrade rather than crash. Both bullets, TOLERANT included — an earlier range in this card stopped five lines short of it and cited only the strict half. | the `THE TWO READ POLICIES ARE NOT AN ACCIDENT` section | [durable_store.py](agents-remember/mcp/src/agents_remember/controlplane/durable_store.py) |
| The leaf's most important statement, and the one this card's read-policy body paraphrases: the three tolerant stores drop an unparseable row permanently rather than for one tick, which is a cost and not a defect only because none of the three carries authority — and if one ever does, its rewrite must be moved onto a strict read first, in the same change. | the `DO NOT GENERALISE "EVERY REWRITE READS STRICTLY" TO ALL SIX` block | [durable_store.py](agents-remember/mcp/src/agents_remember/controlplane/durable_store.py) |
| The version rule as implemented: the major is compared for equality, so `"0.9"` is refused exactly as `"2.0"` is and an unparseable version is refused outright; `DurableRecord` validates `schemaVersion` on the way in so neither reader needs a version branch. | `schema_version_supported`; `SUPPORTED_SCHEMA_MAJOR`; `DurableRecord` | [durable_store.py](agents-remember/mcp/src/agents_remember/controlplane/durable_store.py) |
| `declare_process_role` and `declared_process_role`: the opt-in declaration the two advisory checks read, absent in every CLI invocation and test. Its docstring names the three call sites and states why `_dev_app` is the deliberate exception and the only factory that declares. | `declare_process_role`; `declared_process_role` | [durable_store.py](agents-remember/mcp/src/agents_remember/controlplane/durable_store.py) |
| `StoreOwnership` with no `serialized` field, `check_declared_writer` which raises only inside a declared process, and `is_compaction_owner` which is a question and never throws — including why the undeclared-is-owner default was re-decided and kept rather than inverted. | `StoreOwnership`; `StoreOwnership.check_declared_writer`; `StoreOwnership.is_compaction_owner` | [durable_store.py](agents-remember/mcp/src/agents_remember/controlplane/durable_store.py) |
| The ownership register: all six constants side by side, four logs written by both processes and two by the dashboard alone. All six are named here because no single range covers them. | `GATE_OWNERSHIP`; `EXPECTATION_ROW_OWNERSHIP`; `ATTENTION_DISMISSAL_OWNERSHIP`; `OPERATOR_INBOX_OWNERSHIP`; `ORCHESTRATION_NUDGE_OWNERSHIP`; `SUPERVISOR_SIGNAL_OWNERSHIP` | [durable_store.py](agents-remember/mcp/src/agents_remember/controlplane/durable_store.py) |
| `OPERATOR_INBOX_OWNERSHIP` carries `compaction_owner=None`, the leaf's declared exception, because both processes must physically remove rows. | `OPERATOR_INBOX_OWNERSHIP` | [durable_store.py](agents-remember/mcp/src/agents_remember/controlplane/durable_store.py) |
| `ATTENTION_DISMISSAL_OWNERSHIP` records why a single-writer store is still locked, naming the 31.45 percent an unlocked draft measured. | `ATTENTION_DISMISSAL_OWNERSHIP` | [durable_store.py](agents-remember/mcp/src/agents_remember/controlplane/durable_store.py) |
| `thread_mutex_for` states that `flock` already excludes two threads of one process, so the mutex closes a plausible regression rather than a reproducible loss, and explains why it is re-entrant. Its account of how the six shape their reclaims is the one recorded under Todos. | `thread_mutex_for` | [durable_store.py](agents-remember/mcp/src/agents_remember/controlplane/durable_store.py) |
| `_verify_lock_capability` takes the lock twice from two file descriptions and raises `UnsafeLockFilesystemError` when the second acquisition succeeds. | `_verify_lock_capability` | [durable_store.py](agents-remember/mcp/src/agents_remember/controlplane/durable_store.py) |
| `exclusive_access` takes the per-log mutex before the flock, and the thread-local `_LockDepth` counter makes a nested acquisition return before either lock is touched. `lock_path_for` names the lockfile after the whole log and states why renaming it makes a rolling restart unsafe, with no compatibility path. | `exclusive_access`; `_LockDepth`; `lock_path_for` | [durable_store.py](agents-remember/mcp/src/agents_remember/controlplane/durable_store.py) |
| `require_lock_held` raises from inside `rewrite_lines`, so no store can rewrite a log it has not locked however the call was reached. | `require_lock_held`; `rewrite_lines` | [durable_store.py](agents-remember/mcp/src/agents_remember/controlplane/durable_store.py) |
| The one read both policies share; the only append in the package, which fsyncs before the handle closes; and the only rewrite, which never unlinks the log, uses a pid-scoped hidden temp, and fsyncs both the temp and the parent directory. | `read_log_text`; `append_line`; `rewrite_lines` | [durable_store.py](agents-remember/mcp/src/agents_remember/controlplane/durable_store.py) |
| The MCP process declares its role at the true entry point, not in the `create_server` factory a test would call in-process. | `main` L34-L57, with `declare_process_role("mcp")` at L52 | [mcp/server.py](agents-remember/mcp/src/agents_remember/mcp/server.py) |
| The dashboard declares its role in `run`, for the same reason. | `run` L161-L167, with `declare_process_role("dashboard")` at L167 | [cli/dashboard.py](agents-remember/mcp/src/agents_remember/cli/dashboard.py) |
| The third call site and the only factory that declares: `--reload` serves from a uvicorn `multiprocessing` spawn child that re-imports the module with an empty declaration dict and never reaches `run`, so it answered owner for every log. The docstring records this as an ownership gap and not a durability defect — the unconditional lock covered the rewrite — and why `create_app` still must not declare. | `_dev_app` L52-L81, with `declare_process_role("dashboard")` at L72 | [cli/dashboard.py](agents-remember/mcp/src/agents_remember/cli/dashboard.py) |
| `_reclaim_gate_log` guards on `is_compaction_owner` before compacting, because the dashboard calls `gate_decide_payload` directly; without the guard an MCP-side reclaim runs inside the dashboard. | `_reclaim_gate_log` L453-L473, with the guard at L470 | [mcp/tools/gates.py](agents-remember/mcp/src/agents_remember/mcp/tools/gates.py) |
| The dashboard's HTTP dismiss route calls `AttentionDismissalStore.dismiss`, the whole-file read-modify-write that made the single-writer store the worst loser. | L1164 | [serving/app.py](agents-remember/mcp/src/agents_remember/serving/app.py) |
| `read_gates` no longer rewrites on the projection tick; it uses the tolerant `projected_current`, and physical reclamation moved to the gate log's owner. | `read_gates` L514-L537 | [observer/snapshots.py](agents-remember/mcp/src/agents_remember/observer/snapshots.py) |
| The worktree series contract imports this module's `SCHEMA_VERSION` and `schema_version_supported`, so the tree carries one version policy rather than two. | L16; L36; L891-L896 | [worktrees/worktree_contract.py](agents-remember/mcp/src/agents_remember/worktrees/worktree_contract.py) |
| The control-plane lockfiles are excluded from the projection watch by a rule DERIVED from `lock_path_for` rather than spelled out, and matched by suffix in every watched directory — which is what covers the per-lifecycle `gates.jsonl.lock` a basename list structurally could not. `_EXCLUDED_WORKSPACE_NAMES` no longer names any lockfile; the comment above the derived constant records that spelling it out is exactly what broke. | `_DURABLE_LOG_LOCK_SUFFIX` L142-L156, matched in `is_projection_input_event` at L197 | [serving/change_watcher.py](agents-remember/mcp/src/agents_remember/serving/change_watcher.py) |
