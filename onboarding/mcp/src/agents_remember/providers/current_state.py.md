...
| Unit tests assert the file path, current truth shape, disabled-provider behavior, workflow-local instance paths, and provider-status integration in `ProviderCurrentStateTests`. | "class ProviderCurrentStateTests(unittest.TestCase):" | mcp/tests/test_provider_current_state.py:23-171 |
