...
| The critical-failsafe wiring supplies the stop action directly from the dashboard loop. | "stop_provider_stacks=partial(" | mcp/src/agents_remember/serving/_app_lifespan.py:65-65 |
...
| The supervisor-signal control-plane log whose unlocked draft measured 31.45% loss — the precedent that refused "one writer" as a reason not to lock. | "class AgentNotifierSignalCooldownStore" | mcp/src/agents_remember/controlplane/agent_notifier_signals.py:73-73 |
