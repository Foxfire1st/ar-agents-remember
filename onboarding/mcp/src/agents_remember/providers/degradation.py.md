...
| The critical-failsafe wiring supplies the stop action directly from the dashboard loop. | "stop_provider_stacks=partial(" | mcp/src/agents_remember/serving/_app_lifespan.py:61-61 |
| The inbox record schema (`system-specialist` in `AgentRole`, `degradation-alert` in `InboxMessageKind`) this module posts against. | "degradation-alert" | mcp/src/agents_remember/controlplane/operator_inbox_records.py:52-52 |
