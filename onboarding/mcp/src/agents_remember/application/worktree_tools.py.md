...
`worktree_checkpoint_landing_tool(config, *, contract_path, strategy="ff-only", dry_run=False)` cit:([`worktree_checkpoint_landing_tool`], mcp/src/agents_remember/application/worktree_tools.py:454-494) is the application entry point for the
...
| Finding | Anchor | Source |
| --- | --- | --- |
| Integration, checkpoint, landing recording, and resume normalize code/memory authority without ledger subjects. | `worktree_integrate_tool`; `worktree_checkpoint_landing_tool` | mcp/src/agents_remember/application/worktree_tools.py:371-422; mcp/src/agents_remember/application/worktree_tools.py:454-494 |
