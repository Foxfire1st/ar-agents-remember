...
mints are keyed on the **repository's own namespace** — the stable half — and never on that base, so
the obligation the first task recorded is the same record when the next task reads the line at a later
baseline. The split is what lets a leaf add and modify files and still cite them, and what keeps an
identity stable as the line advances.
...
**The identities one target's own place mints are minted together, and the symbol is what makes two
constructs two records (this leaf's CYCLE-01 repair).** `_identity` derives every id as a `uuid5` under
the one fixed `_INGEST_NAMESPACE` over **the repository's own `repository_id`**, which identity it is
(invariant, revision, route, anchor, claim) and the entry's local id. Keying that derivation on the
enclosure's recorded code base commit — which is what it used to do — made one repository's knowledge a
function of the baseline it happened to be read at: the same obligation under the same local label was a
different record at each baseline, while two different repositories that shared a base commit were handed
the **same** record identity. `ingest_curator_list` therefore resolves the repository identity **before**
planning, so one value reaches every mint in the run instead of one value per step, and `_Source` carries
it to the planner. For a target, `_target_identities` returns the three ids as one `_TargetIdentities`
value, because they are one decision — which construct inside the file the citation is about — applied to
each identity the write path needs. The **route** stays keyed on the written path: a route is a scope that
governs a path, so N anchors in one file are N associations with the one route row rather than N rows. The
**anchor** and the **claim** additionally carry the locator's qualified name (`within`) for a symbol,
which is what makes `resolve_budget` and `other` in one `pkg/module.py` two records: with both minted from
the path and the entry id alone, the second one reached the batch as the same `source_anchor` and was
refused `duplicate_identity`. A range or a whole-file citation contributes no `within`, which is its
existing behaviour and keeps a range citation stable as lines move.

**The report's own identity sentence says the rule the code follows, and that sentence was the last place the old rule survived.** `IngestReport.derived_identities` is a public field a caller reads to understand what an id means, and it still described identity as `uuid5` over the enclosure's recorded code base commit — the exact rule `_identity` had stopped following, and the rule that misled a verification round into reading a minted identity as baseline-scoped. The field now states the fixed `_INGEST_NAMESPACE`, the repository's own namespace identity, which identity it is, the entry's own id and, for a target, what inside the written path the citation is about (a symbol's qualified name, or the locator kind when there is nothing finer), and it names the stable half explicitly as the repository namespace and never the recorded base commit. The module docstring carried the same stale sentence about derived identities staying anchored to the recorded base commit and was corrected with it. What makes the sentence checkable rather than aspirational is the guarded assertion in `test_knowledge_curator_ingest_list.py`: the base commit must not appear in the field, the field must name the repository namespace, and `_identity` is compared against the derivation itself (`uuid5(_INGEST_NAMESPACE, repository_id|kind|entry_id|)`) rather than against a phrase.
...
- **Identities are derived, never random — and the discriminator names the construct.** `uuid5` under
  one fixed `_INGEST_NAMESPACE` over the repository's own `repository_id`, which identity it is
  (invariant, revision, route, anchor, claim) and the entry's own id — plus, for a target, the written
  path when the identity is the route, and the locator's qualified name when the identity is the anchor
  or the claim, because a target carries no identity of its own in revision 1 and a file with two
  constructs has to be able to store two of them: keying an anchor on the path and the entry id alone
  gave two symbols in one source file the same `source_anchor`, which the batch refused after the
  duplicate-**target** guard had already moved. A symbol's qualified name survives the file being edited
  above the definition, where a line number would not; a range and a whole-file citation keep the coarser
  key deliberately, so a range citation stays stable as lines move. The stable half is the repository and
  never the code base commit, because a namespace must not move with a baseline — see
  `_repository_namespace`, `_repository_identity` and `_identity`. Two runs of one list mint the same ids,
  and a second run is diagnosable rather than duplicated.
...
- **The resolution tree moves with the line; the identity does not move with the baseline.** Citations
  ...
  recorded memory base. Every report names both trees. The identities the run mints are keyed on the
  **repository's own `repository_id`**, read from the dataset the run selected and derived only as a
  cold-start fallback, and never on the enclosure's recorded code base commit: a repository's obligation
  has to be the same record at the baseline the next task runs at as at the one that first recorded it,
  and two repositories sharing a base commit must not be handed the same record identity. What still
  separates the *resolution tree* from the *identity* anchor is that resolution is a fact about the line
  and identity is a fact about the repository.
...
while a derived identity is anchored to the repository's own namespace and to neither tree, and the
route leg is two
...
| The symbol boundary and its refusal codes: the qualified-name resolution through the shipped extractor, the locator's language derivation, and the mention-versus-absence distinction — with no second definition implementation beside it. | `_symbol_locator`; `_symbol_language`; `_occurs` | mcp/src/agents_remember/application/knowledge_curator_ingest.py:1940-1983; mcp/src/agents_remember/application/knowledge_curator_ingest.py:1986-2027; mcp/src/agents_remember/application/knowledge_curator_ingest.py:2042-2052 |
| The route leg: the distinct scopes, the pre-batch authoring transaction that classifies authored versus reused, the post-batch attachment transaction, each target's route outcome, and the two primitives whose idempotence the leg depends on. | `_author_routes`; `_author_route_rows`; `_attach_routes`; `_attached_outcome`; `_distinct_routes`; `author_route`; `set_governing_route` | mcp/src/agents_remember/application/knowledge_curator_ingest.py:2134-2162; mcp/src/agents_remember/application/knowledge_curator_ingest.py:2176-2203; mcp/src/agents_remember/application/knowledge_curator_ingest.py:2230-2255; mcp/src/agents_remember/application/knowledge_curator_ingest.py:2275-2300; mcp/src/agents_remember/application/knowledge_curator_ingest.py:2165-2173; mcp/src/agents_remember/memory/knowledge/routes.py:276-332; mcp/src/agents_remember/memory/knowledge/routes.py:436-520 |
| The write half of this module: one target plan's stored anchor and citation, the authored realization role, the entry assembly with its conditions and the revision fields it carries, the single batch commit, and the per-entry rendering of a refused batch. | `_TargetPlan`; `_Authored`; `_authored_role`; `_curator_entry`; `_commit`; `_batch_refused` | mcp/src/agents_remember/application/knowledge_curator_ingest.py:426-478; mcp/src/agents_remember/application/knowledge_curator_ingest.py:620-632; mcp/src/agents_remember/application/knowledge_curator_ingest.py:2461-2476; mcp/src/agents_remember/application/knowledge_curator_ingest.py:2333-2347; mcp/src/agents_remember/application/knowledge_curator_ingest.py:2361-2372; mcp/src/agents_remember/application/knowledge_curator_ingest.py:2405-2433 |
| **The three identities one target's own place mints, derived together rather than apart (this leaf's CYCLE-01 repair).** `_TargetIdentities` is the frozen trio a target needs; `_target_identities` mints them together from the repository identity, the entry's local id, the written path for the route and the locator's qualified name for the anchor and the claim, so two constructs in one `pkg/module.py` are two records while both stay associations with the one route row; and `_identity` is the one derivation, keyed on `repository.repository_id` and never on the enclosure's recorded base commit. | `_TargetIdentities`; `_target_identities`; `_identity` | mcp/src/agents_remember/application/knowledge_curator_ingest.py:479-490; mcp/src/agents_remember/application/knowledge_curator_ingest.py:493-513; mcp/src/agents_remember/application/knowledge_curator_ingest.py:2436-2458 |
| The anchor id the observation is handed, whose discriminator names what within the file the locator points at rather than only the kind of thing it is. | `_observation_id` | mcp/src/agents_remember/application/knowledge_curator_ingest.py:2108-2131 |
