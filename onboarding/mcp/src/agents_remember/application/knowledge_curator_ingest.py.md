...
## Update History
- 2026-09-20T20:56+02:00 — 260915-KS-L48 curator (uncommitted change set on `ar/260915-ks-l48-ar`, code base `f745e16659c5602252bb185a2ffccc356c2bde26`, memory base `c0e559efdf9f3b6ac81be1925329b04c06d27b4c`): **the 22 enforced citation rows on this card (19 distinct anchors) re-read against the committed bytes and re-cited, one deleted construct re-bound, and the body prose that still described that deleted reader corrected.** Every range was re-derived from the construct's real extent in the file the claim already cites, never by adding a delta to an old number: `_ruling_plan` `1879-1899` → `1912-1932`, `_counts` `3417-3455` → `3516-3554`, `_confined` `2003-2009` → `2036-2042`, `_language_of` `2623-2632` → `2656-2665`, `_attach_routes` `2834-2859` → `2937-2962`, `_attached_outcome` `2879-2904` → `2982-3007`, `_authored_role` `3079-3094` → `3182-3197`, `_curator_entry` `2937-2951` → `3040-3054`, `_commit` `2965-2976` → `3068-3079`, `_batch_refused` `3009-3032` → `3112-3135`, `_stored_revisions` `1701-1717` → `1734-1750`, `_with_replays` `1720-1727` → `1753-1760`, `_replayed_outcome` `3138-3155` → `3241-3254`, `_named_anchor_id` `2093-2115` → `2126-2148`, `_identity` `3040-3076` → `3143-3179`, `derived_identities` `440-440` → `450-450`, and the `test_the_governing_route_is_recorded_once_per_scope_and_attached_to_each_anchor` case `1261-1317` → `1318-1374`. `_projected` `3201-3226` → `3300-3325` was re-pointed as well: the range that had been covering it did so only through a docstring mention inside the range re-pointed for `_counts`, so the construct's own declaration had to replace it. Two names in this card pointed at constructs this leaf deleted: `_replayed_routes` exists nowhere in the tree, so the claim now names `_stored_route_outcomes` (`2802-2833`), the read a replay's receipt is composed from, and the body paragraph that still said a replay's routes were seeded "by `_replayed_routes`" now states the current mechanism — the stored association read once over a read-only connection, the path taken back from the route's own row with `routes.route_path_for_id`, and the ledger's `answered` side seeded from those outcomes by `_ledger_scopes` rather than from the arriving request. The mechanical fixer was not run on this document: it declines every one of these anchors with `projection_no_resolved_extent` or `anchor_ambiguous`, so no generated repair bullet is written and no claim here is reopened on this edit's account.
- 2026-09-20T17:41:44+00:00: Generated citation repair: `IngestReport`; `IngestCounts`; `EntryOutcome`; `TargetOutcome`; `RouteOutcome`; `_counts`; `written`; `_projected` repointed to mcp/src/agents_remember/application/knowledge_curator_ingest.py:411-451; mcp/src/agents_remember/application/knowledge_curator_ingest.py:387-408; mcp/src/agents_remember/application/knowledge_curator_ingest.py:373-384; mcp/src/agents_remember/application/knowledge_curator_ingest.py:347-360; mcp/src/agents_remember/application/knowledge_curator_ingest.py:363-370; mcp/src/agents_remember/application/knowledge_curator_ingest.py:3417-3455; mcp/src/agents_remember/application/knowledge_curator_ingest.py:3308-3337; mcp/src/agents_remember/application/knowledge_curator_ingest.py:3201-3226. No content impact: mechanical anchor-range projection bound to citation source snapshot 70d269e55925d3433dfa46b875de4a03ee95578ac949f00908b6206f566496d6; claim bytes unchanged; generated by ccr-r10@v1.
- 2026-09-20T17:23:31+00:00: Generated citation repair: `_plan_target`; "No failure is an exception." repointed to mcp/src/agents_remember/application/knowledge_curator_ingest.py:1902-1941; mcp/src/agents_remember/application/knowledge_curator_ingest.py:58-58. No content impact: mechanical anchor-range projection bound to citation source snapshot 02d8f0b256fe50bf7458ae56160b7e02e4466423e76d3ab197e12477f370f5b8; claim bytes unchanged; generated by ccr-r10@v1.
- 2026-09-20T17:23:31+00:00: Generated citation repair: `COMMITTED`; `REFUSED`; `SKIPPED`; `_ruling_plan` repointed to mcp/src/agents_remember/application/knowledge_curator_ingest.py:180-180; mcp/src/agents_remember/application/knowledge_curator_ingest.py:181-181; mcp/src/agents_remember/application/knowledge_curator_ingest.py:182-182; mcp/src/agents_remember/application/knowledge_curator_ingest.py:1879-1899. No content impact: mechanical anchor-range projection bound to citation source snapshot 02d8f0b256fe50bf7458ae56160b7e02e4466423e76d3ab197e12477f370f5b8; claim bytes unchanged; generated by ccr-r10@v1.
- 2026-09-20T17:23:31+00:00: Generated citation repair: `_TargetIdentities`; `_target_identities`; `_Authoring`; `_identity` repointed to mcp/src/agents_remember/application/knowledge_curator_ingest.py:549-561; mcp/src/agents_remember/application/knowledge_curator_ingest.py:632-684; mcp/src/agents_remember/application/knowledge_curator_ingest.py:618-629; mcp/src/agents_remember/application/knowledge_curator_ingest.py:3040-3076. No content impact: mechanical anchor-range projection bound to citation source snapshot 02d8f0b256fe50bf7458ae56160b7e02e4466423e76d3ab197e12477f370f5b8; claim bytes unchanged; generated by ccr-r10@v1.
- 2026-09-20T17:17:10+00:00: Generated citation repair: `_observation_id` repointed to mcp/src/agents_remember/application/knowledge_curator_ingest.py:2701-2719. No content impact: mechanical anchor-range projection bound to citation source snapshot 02d8f0b256fe50bf7458ae56160b7e02e4466423e76d3ab197e12477f370f5b8; claim bytes unchanged; generated by ccr-r10@v1.

- 2026-09-20T18:45:00+02:00 — 260915-KS-L47 worker (uncommitted change set on `ar/260915-ks-l47-ar`, code base `be325216416326a66950c9e320ff8d08f41e5d66`): **two write-binding repairs and one withdrawn universal, in the intake.** (1) `_content_digest` now digests the whole semantic write intent — it gained `dispositionSource`, `namedInvariantId`, `predecessors`, `role` and `roleRationale` — because the five omitted fields were exactly the ones a changed request could ride a held retry key through: measured on this base, changing `realization_role`+`realization_rationale`, or `disposition_source`, or `invariant_id`+`predecessor_revision_ids` each returned exit 0 / `batchState: replayed` / committed / `refused: []` / `recordsWritten: 0` against a byte-identical database, and the disposition-source report echoed the new ruling text while the stored revision kept the old one. All three now refuse `allocation_content_conflict` with the database preserved, and the identical retry still replays with its original ids. The correction block above records why the paragraph's former "every fact the stored truth is made of" universal is withdrawn rather than reworded, and names the two deliberate exclusions (`entry_id` as the retry key's own scoping half, and the derived `declares_invariant`). (2) An explicitly reused `anchor_id` is now bound to the stored row it names: `_require_stored_anchor` compares the supplied path, blob and locator against the stored anchor **before the plan exists**, so a target naming symbol `other` while citing the `resolve_budget` anchor is refused `anchor_reuse_mismatch` instead of committing and reporting a locator the database does not link; an identity no dataset holds is refused `anchor_id_not_stored`. `_Source` gained the `anchors` dataset tuple the read resolves against, in candidate-then-baseline order. **Stamp accounting:** `reviewedWorkingCandidate` now names this leaf's candidate on base `be325216` and `lastUpdated` moved with it; the `lastVerifiedCommitHash`/`lastVerifiedCommitDate` pair is retained exactly as recorded, because no commit contains this body and no stamp was measured on it. No commit was made.
