...
| The honest report when no detection run is recorded, and the report of one run's conditions, limitations and unresolved items with no verdict either way. | `_no_detection_run`; `_condition_report` | mcp/src/agents_remember/mcp/tools/knowledge.py:336-365 |
| The project builder: the destination profile, the refusal when no view was named, the delegation to the projection operation, and the per-path report relayed back. | `knowledge_project_payload`; `_refused_project` | mcp/src/agents_remember/mcp/tools/knowledge.py:368-407; mcp/src/agents_remember/mcp/tools/knowledge.py:434-442 |
