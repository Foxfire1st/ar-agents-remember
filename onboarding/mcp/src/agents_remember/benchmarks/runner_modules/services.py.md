...
| Benchmark behavior is covered through the existing worktree/tool test slices. | `WorktreeSupportTests` | mcp/tests/test_worktree_support.py:708-783 |
...
- 2026-09-17T03:31:11+02:00 — 260915-KS-L9 curator (re-scoped repair): re-read the reopened claim in the row 71 of this card against the current code: its anchor still resolves inside the cited range, so the wording holds and the stamp advances
