...
The former shipped-vocabulary comparison test was retired. The pinned digest remains a
production integrity check, but that removed test no longer detects dependency/hash drift.
...
| `_tool_payload` finalizes token metadata on every public tool response via this module. | `_tool_payload` | mcp/src/agents_remember/mcp/tools/base.py:22-24 |
| `TokenizerVocabularyError` — the typed refusal raised instead of downloading a vocabulary this package does not ship. | `TokenizerVocabularyError` | mcp/src/agents_remember/errors.py:335-343 |
...
- 2026-09-04T01:48+02:00 — 260831-CCR-L08 Gate-5 memory pass: re-anchored the tokenizer errors.py row (284-292 to 317-325) shifted by the CCR-R08 +33-line errors.py insertion. Citation-only re-anchor; no content impact.
