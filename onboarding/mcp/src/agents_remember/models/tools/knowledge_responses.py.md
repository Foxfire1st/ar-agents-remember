...
| The change handler whose two reachable branches answer `refused` with `registration_absent`, and the admitted kinds it checks against. | `knowledge_change_payload`; `ADMITTED_CHANGE_KINDS` | mcp/src/agents_remember/mcp/tools/knowledge.py:198-229; mcp/src/agents_remember/mcp/tools/knowledge.py:59-67 |
| The diff and integrity handlers: only caller-supplied labels survive, and the integrity payload carries no verdict. | `_supplied_effect_labels`; `knowledge_integrity_check_payload` | mcp/src/agents_remember/mcp/tools/knowledge.py:267-281; mcp/src/agents_remember/mcp/tools/knowledge.py:284-317 |
