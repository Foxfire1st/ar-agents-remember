...
| The change handler that answers `refused` with `registration_absent` for every kind -- declared here or not -- and the declared kinds it names beside that refusal. | `knowledge_change_payload`; `DECLARED_CHANGE_KINDS` | mcp/src/agents_remember/mcp/tools/knowledge.py:287-315; mcp/src/agents_remember/mcp/tools/knowledge.py:91-98 |
...
| The project handler that returns the per-path published/retained/discrepancy lists and the manifest generation. | `knowledge_project_payload` | mcp/src/agents_remember/mcp/tools/knowledge.py:558-561 |
| The one registered shape it is a payload of, where the state/payload exclusion is a validator rather than handler discipline. | `ViewPayload`; `ViewResult`; `_require_one_outcome` | mcp/src/agents_remember/models/knowledge/change_set.py:280-287; mcp/src/agents_remember/models/knowledge/view.py:1079-1094; mcp/src/agents_remember/models/knowledge/view.py:901-945 |
