...
| The three newest advertised names, appended at the tail with the registrar and registry rows that publish them. | `role_capsule_compile`; `skill_catalog_list`; `skill_catalog_read` | mcp/src/agents_remember/models/tools/public_roster.py:86-89 |
...
| The registrar that publishes the three newest names, whose order the tuple must match. | `register_capsule_and_skill_tools` | mcp/src/agents_remember/mcp/registration/capsule_serving.py:72-77 |
