...
| `lifecycle_finalize_task_tool` populates `autoLandedSeats` from `_auto_land_completed_seats`, gated by `config.retirement.auto_land_on_finalize`. | "def lifecycle_finalize_task_tool(" | mcp/src/agents_remember/application/worktree_tools.py:912-912 |
