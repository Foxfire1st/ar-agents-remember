...
`VIEW_PURPOSES: Mapping[str, str]` supplies one published phrase per view for the interface `KS-R22@v1` mounts
...
Declarations that must agree are one declaration. `VIEW_NAMES` is `get_args(ViewName)`; `COUNT_QUANTITIES` is
...
- **Five views and no sixth.** `ViewName` is a five-member `Literal`, `VIEW_NAMES` is `get_args` of it, each
...
| The four inputs a completeness statement is scoped to and the field spelled so it cannot claim project semantics. | `ViewScope`; `complete_within_declared_scope` | mcp/src/agents_remember/models/knowledge/view.py:339-351; mcp/src/agents_remember/models/knowledge/view.py:354-363 |
