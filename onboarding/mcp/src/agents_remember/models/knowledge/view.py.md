...
| The four per-view row shapes with their closed fact-kind literals, the member-record-versus-attributed-source locus, the assessment and source-change states and the conditions-omitted flag. | `FamilyRow`; `change_locus`; `conditions_omitted`; `source_change_state` | mcp/src/agents_remember/models/knowledge/view.py:752-926; mcp/src/agents_remember/models/knowledge/view.py:922-922; mcp/src/agents_remember/application/knowledge_view_render.py:66-66; mcp/src/agents_remember/application/knowledge_view_render.py:1021-1021; mcp/src/agents_remember/application/knowledge_view_render.py:1042-1042; mcp/src/agents_remember/application/knowledge_view_render.py:1059-1059; mcp/src/agents_remember/application/knowledge_view_render.py:1064-1064; mcp/src/agents_remember/application/knowledge_view_render.py:1071-1071 |
| The machine-selected work item with no judgment field, the separately attributed curator disposition with no detector field, and the row composing them. | `MachineWorkItem`; `CuratorDisposition`; `CurationQueueRow` | mcp/src/agents_remember/models/knowledge/view.py:862-874; mcp/src/agents_remember/models/knowledge/view.py:877-889; mcp/src/agents_remember/models/knowledge/view.py:892-898; mcp/src/agents_remember/application/knowledge_view_render.py:69-69; mcp/src/agents_remember/application/knowledge_view_render.py:65-65; mcp/src/agents_remember/application/knowledge_view_render.py:64-64; mcp/src/agents_remember/application/knowledge_view_render.py:1154-1154; mcp/src/agents_remember/application/knowledge_view_render.py:1168-1168; mcp/src/agents_remember/application/knowledge_view_render.py:1185-1185; mcp/src/agents_remember/application/knowledge_view_render.py:1190-1190; mcp/src/agents_remember/application/knowledge_view_render.py:1207-1207 |
| The payload validator that ties rows-remaining to the continuation, the continuation to the view and snapshot, the completeness scope to the payload's snapshot, and every ordering rule to the registry. | `ViewPayload`; `_require_honest_bounding` | mcp/src/agents_remember/models/knowledge/view.py:934-978; mcp/src/agents_remember/models/knowledge/view.py:952-978; mcp/src/agents_remember/application/knowledge_projection.py:54-54; mcp/src/agents_remember/application/knowledge_projection.py:90-90; mcp/src/agents_remember/application/knowledge_projection.py:105-105; mcp/src/agents_remember/application/knowledge_projection.py:187-187 |
| The discriminated union that keeps each payload's concrete row type, the declared page size, and the tuple of the five payload classes. | `ViewPayloadUnion`; `MAX_VIEW_ROWS`; `VIEW_PAYLOADS` | mcp/src/agents_remember/models/knowledge/view.py:97-97; mcp/src/agents_remember/models/knowledge/view.py:1018-1021; mcp/src/agents_remember/models/knowledge/view.py:1025-1025; mcp/src/agents_remember/models/knowledge/view.py:1027-1033 |
| The reader port as a runtime-checkable protocol of five methods — including the membership read a generic kind lookup cannot answer — and the recorded row and registered totals it hands a view. | `KnowledgeViewReader`; `family_member_rows`; `ViewSourceRow`; `ViewSourceCounts` | mcp/src/agents_remember/models/knowledge/view.py:1041-1058; mcp/src/agents_remember/models/knowledge/view.py:1061-1065; mcp/src/agents_remember/models/knowledge/view.py:1068-1114; mcp/src/agents_remember/models/knowledge/view.py:1101-1110; mcp/src/agents_remember/application/knowledge_view_render.py:520-520; mcp/src/agents_remember/application/knowledge_view_render.py:741-741 |
...
| The three optional location fields a realization row carries — the recorded role, path and locator, reported by the invariant and the family view alike — and the one stored locator both decode through. | `RealizationRole`; `SourceLocator`; `locator` | mcp/src/agents_remember/models/knowledge/view.py:782-819; mcp/src/agents_remember/models/knowledge/view.py:822-859; mcp/src/agents_remember/models/knowledge/view.py:770-770 |
...
## Update History
- 2026-09-21T12:40+02:00 — 260915-KS-L44 curator (uncommitted CYCLE-03 change set on `ar/260915-ks-l44-ar`, code base `2a96eb88`, memory base `9df7f499`): **body update for the two row models this leaf completes, and the citation shift its own edit caused — including on three cards outside this leaf's change set.** (a) `InvariantRow` (`mcp/src/agents_remember/models/knowledge/view.py:782-819`) and `FamilyRow` (`:822-859`) now declare `role: RealizationRole | None`, `path: str | None` (bounded by `REFERENCE_MAX_LENGTH`) and `locator: SourceLocator | None`: a realization row reported the claim id, the invariant revision id and the authored rationale and no place at all, so a caller could see that a realization existed and not where it is. The recorded location was already on the candidate the projection was handed, so nothing is derived and no filename is read out of authored prose — the Logic section gained the paragraph, the Invariants section the bullet, and the reference table the row for the three fields. Both models are one shape behind more than one row kind, so the fields appear on every row of their view as explicit `null`s, the convention this module already had for `SourceContextRow.anchor_state`. `ReviewMatrixRow` and `CurationQueueRow` gain nothing, and the card now says why: no candidate feeding either view sets a `path` or a `locator`, so there is no dropped answer there. (b) **Citation repair, all of it this leaf's own `+28`** — the two field blocks sit above every declaration below them, so no claim was re-worded and no anchor or row dropped: the four-row-shapes row `756-894` → `752-926` with `source_change_state` `894-894` → `922-922`; the queue-row row `864-870; 849-862; 834-847; 867-868` → `862-874; 877-889; 892-898`, with the renderer's own holds re-read rather than shifted (`CurationQueueRow` at `64`, `1154`, `1168`, `1185`, `1190`, `1207`; `CuratorDisposition` at `65`, `1197`; `MachineWorkItem` at `69`, `1191`, `1208`); the payload-validator row `728-772; 906-906; 907-932; 888-932` → `934-978; 952-978`; the union row `97-97; 990-993; 997-997; 999-1005` → `97-97; 1018-1021; 1025-1025; 1027-1033`; the port row `1013-1030; 1033-1037; 1041-1086; 1073-1083` → `1041-1058; 1061-1065; 1068-1114; 1101-1110`, with `family_member_rows` in the renderer at `520` and `741`; the request/result row `1089-1120; 1123-1138` → `1117-1148; 1151-1166`; and the path-seed row `1089-1120; 1102-1102; 1108-1120` → `1117-1148; 1130-1130; 1136-1148`. (c) **Three cards outside this leaf's change set cite this file by line and were re-read and repointed in the same pass rather than left to rot**: `application/knowledge_views.py.md` (three rows — `339-363; 728-772; 888-932` → `353-365; 934-978`; `953-957; 981-985; 999-1005; 1041-1086` → `981-985; 1009-1013; 1027-1033; 1068-1114`; `1078-1111; 1112-1127` → `1117-1148; 1151-1166`), `memory/knowledge/view_source.py.md` (the reader row — `1032-1067; 1073-1082` → `1068-1114; 1101-1110`) and `models/tools/knowledge_responses.py.md` (the registered-shape row — `906-952; 1112-1127; 1122-1127` → `934-978; 1151-1166; 1160-1166`). Their wording was retained in every case; only the ranges moved. `lastVerifiedCommitHash`/`lastVerifiedCommitDate` are retained as recorded — the candidate is uncommitted and the governed closeout owns the real stamp.
- 2026-09-20T05:28+02:00 — 260915-KS-L41 curator (uncommitted change set on `ar/260915-ks-l41-ar`, code base `756c47b3` at this leaf's cut and `f79f4db745ad00b908d6ce4871d0b4ab2320207c` after the L39 sync, memory base `da33325c` at the cut and `37d0787571bfbf92890049ee0614fa159012be39` after it): **body update for the port method this leaf declares, and the two stale citations the report named on this card.** `KnowledgeViewReader` gained `family_member_rows()` (`models/knowledge/view.py:1073-1083`), so the reader-port paragraph, the port invariant and the port's reference row now read **five** methods rather than four and state why the method exists: family membership has its own generation-1 table and is not duplicated into the `knowledge_record`/`record_revision` envelope `rows(record_kind)` reads, so the named-kind lookup answered "no rows" on a dataset that holds membership and the family view reported a guarantee, no members, no locations and a complete answer. Two citation cells were re-read against the candidate rather than shifted: the source-path-seed row cited `PathSeed` at three `view.py` spans that hold only the import, the docstring mention and the validator's return, so the class's own declaration is now cited where it lives (`models/knowledge/read.py:144-171`) beside the request's validator (`view.py:1108-1120`); and the port row's own spans, left over from a layout this module no longer has, now cite the declarations they name. The `ViewRequest`/`_require_one_outcome` row's two own-file cells were repointed the same way. No claim was re-worded to fit a stale pointer, no anchor or range was dropped, `lastVerifiedCommitHash`/`lastVerifiedCommitDate` are retained as recorded, and no stamp was advanced or invented — the candidate is uncommitted and the governed closeout owns the real stamp.
