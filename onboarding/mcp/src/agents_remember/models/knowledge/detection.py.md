...
| The envelope registry these payload models are registered in, which is why the field set is declared once here. | `PAYLOAD_MODELS` | mcp/src/agents_remember/memory/knowledge/record_envelope.py:86-100 |
...
| The refusal type a documented refusal would be carried on. | `KnowledgeRefusal` | mcp/src/agents_remember/models/knowledge/result.py:175-185 |
