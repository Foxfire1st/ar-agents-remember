...
## Update History
- 2026-09-20T05:41+02:00 — 260915-KS-L41 curator (cross-card reconciliation; uncommitted change set on `ar/260915-ks-l41-ar`, code base `756c47b3` before the L39 sync and `f79f4db745ad00b908d6ce4871d0b4ab2320207c` after it): **this card is reconciled, not extended — its one four-anchor row was re-pointed because another leaf's change set moved the cases it cites, and no claim, anchor or body byte of this card changed.** The row names four cases in `mcp/tests/test_knowledge_views_and_projection.py`, a file 260915-KS-L41 modifies: `test_a_path_that_is_not_purely_destination_relative_is_refused` now spans `407-421`, `test_a_case_collision_names_both_paths_and_both_records` `424-436`, `test_a_collision_refuses_the_whole_plan_before_either_output_is_written` `597-614` and `test_the_manifest_refuses_two_owners_for_one_path` `719-733`, each read at its own `def` in the merged candidate rather than offset from the old numbers. The fifth cell is untouched and still holds its anchor in a different file. The confinement, collision-reporting, single-owner and collision-refuses-the-plan protections the claim names are all still what those four cases measure; nothing in the claim's wording depends on a line. `lastVerifiedCommitHash`/`lastVerifiedCommitDate` are retained as recorded — the candidate is uncommitted and the governed closeout owns the real stamp.
- 2026-09-20T01:02:52+02:00 — 260915-KS citation residue clearance (uncommitted change set on memory base `66b2ae8adebea11bc2300d2d51822f321a128657`): verified the 4 enforced citation rows this card carried (citation_anchor_absent_from_range ×4) by hand-reading `mcp/tests/test_knowledge_views_and_projection.py`: the four nodes sit at lines 368, 385, 558 and 680, each inside the range the row now cites (`:368-382`, `:385-397`, `:558-575`, `:680-694`, with the shared fixture at `test_knowledge_projection_vault_safety.py:83-116`), so the earlier mechanical projection was correct and needed no further edit; the claim wording, the anchors and every range are unchanged, and no verification stamp was advanced.
- 2026-09-20T00:31+02:00 — 260915-KS-L30 curator (uncommitted change set on `ar/260915-ks-l30-ar`, base `7dcec036094768c5f50e571fb45e59a27ae78efc`): **mechanical citation-range projection** against this leaf's candidate. The checklist reported 4 row(s) whose cited range no longer holds its anchor although the construct is present in the cited file; each range was widened to the lines that carry it — `test_a_case_collision_names_both_paths_and_both_records`; `test_a_collision_refuses_the_whole_plan_before_either_output_is_written`; `test_a_path_that_is_not_purely_destination_relative_is_refused`; `test_the_manifest_refuses_two_owners_for_one_path`. No claim wording, anchor or citation was added, removed or re-worded, and no range was deleted: the new range is the checklist's own resolved extent for that anchor on this candidate. No verification stamp was advanced.
- 2026-09-19T22:33+02:00 — 260918-TSIP-L11 curator (memory worktree `fd1a024e`, code `7879f5b2`): cleared the inherited citation debt on 1 claim(s) by RE-READING each claim against the merged tree and RE-DERIVING every cited range from the construct's real extent in the file the claim cites (`extents.anchor_extents`), never by adding a delta to an old number and never through the mechanical projection (no generated citation-repair bullet is written, so no claim is reopened by this edit). Claims re-read: `projection_manifest.py.md:251` (`test_a_path_that_is_not_purely_destination_relative_is_refused`, `test_a_case_collision_names_both_paths_and_both_records`, `test_a_collision_refuses_the_whole_plan_before_either_output_is_written`, `test_the_manifest_refuses_two_owners_for_one_path`, `test_the_vault_safety_contract_holds_for_all_eight_checkpoints_in_one_scenario`).
