...
- **A new code is a vocabulary decision, not a raise-site convenience.** The L3 codes were added because two
  ...
  reuses `unauthorized_scope` because it is the same class of failure.
...
| Finding | Anchor | Source |
| --- | --- | --- |
| The declared mutating-operation vocabulary, including the six the L4 leaf added and the two L5 merge operations. | `KnowledgeOperation` | mcp/src/agents_remember/models/knowledge/result.py:36-62 |
| The declared refusal vocabulary, with each member naming a distinct observable failure — now including the seven L4 codes and the twelve L5 merge codes. | `KnowledgeRefusalCode` | mcp/src/agents_remember/models/knowledge/result.py:66-111 |
| The refusal value carrying code, operation, offending record and next action. | `KnowledgeRefusal` | mcp/src/agents_remember/models/knowledge/result.py:114-124 |
| The two shared outcome-consistency rules every result model calls. | `require_stored_outcome`; `require_removal_outcome` | mcp/src/agents_remember/models/knowledge/result.py:127-141; mcp/src/agents_remember/models/knowledge/result.py:144-152 |
| The digest-free caller draft that refuses a self-declared predecessor. | `RevisionDraft` | mcp/src/agents_remember/models/knowledge/result.py:155-178 |
| The two label-edit requests, each naming the row the caller read. | `SetInvariantLabelRequest`; `SetFamilyLabelRequest` | mcp/src/agents_remember/models/knowledge/result.py:272-283; mcp/src/agents_remember/models/knowledge/result.py:286-292 |
| The two label-edit results and their `labeled`/`refused` consistency validators. | `SetInvariantLabelResult`; `SetFamilyLabelResult` | mcp/src/agents_remember/models/knowledge/result.py:477-497; mcp/src/agents_remember/models/knowledge/result.py:500-520 |
