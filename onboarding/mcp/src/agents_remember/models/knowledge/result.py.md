...
`merge_knowledge_datasets`), and **the two L6 added**: `export_knowledge_dataset` and
`import_knowledge_dataset`.
`KnowledgeRefusalCode` declares thirty-seven codes:
...
`changeset_postcondition_failed`), and **the one L6 added: `invalid_export`**.
...
| Finding | Anchor | Source |
| --- | --- | --- |
| The declared mutating-operation vocabulary, including the six L4 operations, the two L5 merge operations and the two L6 portable operations. | `KnowledgeOperation` | mcp/src/agents_remember/models/knowledge/result.py:36-69 |
| The declared refusal vocabulary, with each member naming a distinct observable failure — now including the seven L4 codes, the twelve L5 merge codes and the one L6 portable code. | `KnowledgeRefusalCode` | mcp/src/agents_remember/models/knowledge/result.py:72-120 |
| The refusal value carrying code, operation, offending record and next action. | `KnowledgeRefusal` | mcp/src/agents_remember/models/knowledge/result.py:127-138 |
| The two shared outcome-consistency rules every result model calls. | `require_stored_outcome`; `require_removal_outcome` | mcp/src/agents_remember/models/knowledge/result.py:140-155; mcp/src/agents_remember/models/knowledge/result.py:157-166 |
| The digest-free caller draft that refuses a self-declared predecessor. | `RevisionDraft` | mcp/src/agents_remember/models/knowledge/result.py:168-195 |
| The two label-edit requests, each naming the row the caller read. | `SetInvariantLabelRequest`; `SetFamilyLabelRequest` | mcp/src/agents_remember/models/knowledge/result.py:285-297; mcp/src/agents_remember/models/knowledge/result.py:299-321 |
| The two label-edit results and their `labeled`/`refused` consistency validators. | `SetInvariantLabelResult`; `SetFamilyLabelResult` | mcp/src/agents_remember/models/knowledge/result.py:490-511; mcp/src/agents_remember/models/knowledge/result.py:513-534 |
