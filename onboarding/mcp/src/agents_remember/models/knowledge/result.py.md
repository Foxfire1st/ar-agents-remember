...
`import_knowledge_dataset`), and **the one L7 added: `read_knowledge_scope`** — one operation rather than two
because a seed and a continuation are two ways of asking the same question ("which recorded scope, and which
page of it") and a caller branches on the refusal code, not on which of the two it passed. The measured union
is therefore **twenty-five operations**.
`KnowledgeRefusalCode` declares **forty-four codes** (measured by counting the literal's members; the
thirty-seven this card recorded before this pass was already one short of the thirty-eight the union held at
this leaf's own base):
...
`snapshot_unavailable`, `selection_incomplete`.
...
| Finding | Anchor | Source |
| --- | --- | --- |
| The declared mutating-operation vocabulary, including the six L4 operations, the two L5 merge operations, the two L6 portable operations and **the one L7 read operation** — twenty-five members, measured. | `KnowledgeOperation` | mcp/src/agents_remember/models/knowledge/result.py:36-72 |
| The declared refusal vocabulary, with each member naming a distinct observable failure — the seven L4 codes, the twelve L5 merge codes, the one L6 portable code and **the six L7 read codes**; forty-four members, measured. | `KnowledgeRefusalCode` | mcp/src/agents_remember/models/knowledge/result.py:76-141 |
...
| The refusal value carrying code, operation, offending record and next action. | `KnowledgeRefusal` | mcp/src/agents_remember/models/knowledge/result.py:131-155 |
| The two shared outcome-consistency rules every result model calls. | `require_stored_outcome`; `require_removal_outcome` | mcp/src/agents_remember/models/knowledge/result.py:157-172; mcp/src/agents_remember/models/knowledge/result.py:174-183 |
| The digest-free caller draft that refuses a self-declared predecessor. | `RevisionDraft` | mcp/src/agents_remember/models/knowledge/result.py:185-212 |
| The two label-edit requests, each naming the row the caller read. | `SetInvariantLabelRequest`; `SetFamilyLabelRequest` | mcp/src/agents_remember/models/knowledge/result.py:302-314; mcp/src/agents_remember/models/knowledge/result.py:316-338 |
| The two label-edit results and their `labeled`/`refused` consistency validators. | `SetInvariantLabelResult`; `SetFamilyLabelResult` | mcp/src/agents_remember/models/knowledge/result.py:507-528; mcp/src/agents_remember/models/knowledge/result.py:530-551 |
