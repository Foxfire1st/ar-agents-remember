...
| The application assigns the operation identity and the recorded instant instead of accepting them. | `write_authorship` | mcp/src/agents_remember/application/knowledge.py:46-66 |
| The envelope is stored as a canonical JSON typed column. | `encode_authorship`; `decode_authorship` | mcp/src/agents_remember/memory/knowledge/records.py:52-57 |
...
| Git commit attribution stays with its existing owner; this leaf read it for context and copied nothing. | — | mcp/src/agents_remember/kernel/memory_attribution.py |
