...
| The unknown-role, unknown-operation, non-applicable-operation and launcher cases. | `test_unknown_role_is_refused_and_never_acquires_a_capsule`; `test_unknown_operation_is_refused_instead_of_falling_back`; `test_operation_the_role_cannot_run_is_refused_instead_of_substituted`; `test_the_launcher_seat_composes_its_own_core_and_inherits_no_role_operation` | mcp/tests/test_role_capsule_compiler.py:526-543; mcp/tests/test_role_capsule_compiler.py:545-551; mcp/tests/test_role_capsule_compiler.py:553-558; mcp/tests/test_role_capsule_compiler.py:561-580 |
...
## Update History
- 2026-09-20T01:29+02:00 — 260915-KS-L30 curator (uncommitted change set on `ar/260915-ks-l30-ar`, base `7dcec036094768c5f50e571fb45e59a27ae78efc`): **cleared the last three citation rows of this card — two dead anchors and the reopened claim that names them (one row).** (a) The case-list row named `test_launcher_seat_composes_its_own_core_and_is_not_a_role` and `test_launcher_is_refused_an_operation_no_role_inherits_to_it`, neither of which exists anywhere in the tree. The source records that they are one case now: `test_the_launcher_seat_composes_its_own_core_and_inherits_no_role_operation` (mcp/tests/test_role_capsule_compiler.py:561) states in its own docstring "The positive shape and the refusal it implies are one rule -- the launcher's composition is its own -- so they are one case: the capsule composes the launcher's core and names no role, and an operation only a role could run is refused rather than substituted." The Anchor cell names that one surviving case — the row's other three anchors and their ranges are untouched — and the two ranges that held the two old cases (`:561-567`, `:570-575`) were merged into the surviving case's own declaration extent `:561-580`: one range for one case, rather than the same range written twice, which the checker refuses as a duplicate source. (b) Re-read against the candidate, the claim's wording **holds as written** — the row still names exactly the unknown-role, unknown-operation, non-applicable-operation and launcher cases. **Stamp accounting:** the stale `lastVerifiedCommitHash`/`lastVerifiedCommitDate` rows were replaced by ONE `reviewedWorkingCandidate` row naming this candidate, because no commit contains the body as it now stands and no stamp was measured on it.
- 2026-09-20T01:00+02:00 — 260915-KS citation residue clearance (uncommitted change set on memory base `66b2ae8adebea11bc2300d2d51822f321a128657`): cleared none of the 3 enforced citation rows this card carried (citation_claim_reopened; citation_anchor_absent_from_range) — all three name `test_launcher_seat_composes_its_own_core_and_is_not_a_role` and `test_launcher_is_refused_an_operation_no_role_inherits_to_it`, which exist nowhere in the tree because the two cases were consolidated into `test_the_launcher_seat_composes_its_own_core_and_inherits_no_role_operation` at mcp/tests/test_role_capsule_compiler.py:561-580; the row's ranges already point inside that case, so no range edit can hold the dead names and a claim-level re-read is required. No range was changed here; every claim wording, anchor and every other range is unchanged, and no verification stamp was advanced.
