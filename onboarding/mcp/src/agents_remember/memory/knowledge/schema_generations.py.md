...
| Generation 2 composed as an explicit append over generation 1, its fingerprint derived from the composition. | `GENERATION_2` | mcp/src/agents_remember/memory/knowledge/schema_generations.py:313-313 |
| **Generation 3 composed as an explicit append over generation 2, and generation 4 as the same append over generation 3 — the prefix equality that is the additive rule.** | `GENERATION_3`; `_compose_generation_4`; `GENERATION_4` | mcp/src/agents_remember/memory/knowledge/schema_generations.py:313-324; mcp/src/agents_remember/memory/knowledge/schema_generations.py:327-338; mcp/src/agents_remember/memory/knowledge/schema_generations.py:341-341 |
...
| Generation 2 composed as an explicit append over generation 1, its fingerprint derived from the composition. | `GENERATION_2` | mcp/src/agents_remember/memory/knowledge/schema_generations.py:313-313 |
| **Generation 3 composed as an explicit append over generation 2, and generation 4 as the same append over generation 3 — the prefix equality that is the additive rule.** | `GENERATION_3`; `_compose_generation_4`; `GENERATION_4` | mcp/src/agents_remember/memory/knowledge/schema_generations.py:313-324; mcp/src/agents_remember/memory/knowledge/schema_generations.py:327-338; mcp/src/agents_remember/memory/knowledge/schema_generations.py:341-341 |
| **The registry, ordered oldest first, and the created generation defined as its last entry rather than as a second literal.** | `GENERATIONS`; `CURRENT_GENERATION` | mcp/src/agents_remember/memory/knowledge/schema_generations.py:459-469; mcp/src/agents_remember/memory/knowledge/schema_generations.py:482-482 |
| The schema name generation 4 declares, and the three names before it. | `GENERATION_4_SCHEMA_NAME` | mcp/src/agents_remember/memory/knowledge/schema_generations.py:221-221 |
| The only place a build's own generation decides anything, and it decides only what a created store declares. | `generation_of_new_store` | mcp/src/agents_remember/memory/knowledge/schema_generations.py:501-504 |
| The drift gate that fails rather than warns, and the generation-1 wrapper. | `require_pinned_generation_unchanged` | mcp/src/agents_remember/memory/knowledge/schema_generations.py:482-492 |
| The one fingerprint definition, now a function of a record rather than of the running build. | `structure_manifest` | mcp/src/agents_remember/memory/knowledge/schema_generations.py:153-163 |
...
## Update History
- 2026-09-18T18:04:10+00:00: 260915-KS-L23 residue clearance (seat A, follow-up): removed a byte-identical repeated Source from the two rows that name `GENERATION_3`, `_compose_generation_4` and `GENERATION_4`: `mcp/src/agents_remember/memory/knowledge/schema_generations.py:341-341` appeared twice in one Source cell and the row now cites it once beside `:313-324` and `:327-338`. `citation_source_duplicate`'s instruction is "Keep one copy: repetition adds no pooled evidence"; the kept copy is the line the `GENERATION_4` anchor is declared at, so every anchor still has its own cited occurrence. No anchor, range, row or claim was deleted and no wording changed. Verification stamp not advanced: the code is uncommitted and closeout owns the stamp.
- 2026-09-18T17:54:35+00:00: 260915-KS-L23 residue clearance (seat A): two rows repointed to the extents their own anchors now occupy, in both copies of the Repo-Internal References table. `GENERATION_3`; `_compose_generation_4`; `GENERATION_4` @ `schema_generations.py:313-324; :327-338` -> `:313-324; :327-338; :341-341; :341-341` — the two new ranges hold `GENERATION_4 = _compose_generation_4()`, so the row's `_compose_generation_4` and `GENERATION_4` anchors each resolve in a range that carries it, beside the `GENERATION_3` definition the two kept ranges hold; `GENERATIONS`; `CURRENT_GENERATION` @ `:456-466; :479-479` -> `:459-469; :482-482` — the new ranges hold the registry tuple `GENERATIONS: tuple[SchemaGeneration, ...] = (GENERATION_1 … GENERATION_9)` and `CURRENT_GENERATION = GENERATIONS[-1]`. Each claim was re-read against the construct its row names; wording unchanged, no anchor, citation, range or claim deleted, and the verification stamp is not advanced: the code is uncommitted and closeout owns the stamp.
...
- 2026-09-18T17:30:57+00:00: Generated citation repair: `GENERATION_2` repointed to mcp/src/agents_remember/memory/knowledge/schema_generations.py:313-313. No content impact: mechanical anchor-range projection bound to citation source snapshot 90ac134ffc3f8e781bc1feb4daa6ea3e6fd982366fb532c5a9c6ca2e3d9aa040; claim bytes unchanged; generated by ccr-r10@v1.
- 2026-09-18T19:28+02:00 — 260915-KS-L23 curator (uncommitted change set on `ar/260915-ks-l23`, base `c5a74a85`): **No content impact: re-read this card's composition claims against the module docstring and the AST, and the change to `schema_generations.py` is a comment correction, not a behavioural one.** The comment above the wrappers (`schema_generations.py:243-250`) now says eight per-generation wrappers follow — generations 2 to 9, seven of them a single `_append_generation` call naming only their base, schema name, version and module, and generation 5 the one hand-written seventeen-line composition that does not go through that call — and the generation-5 comment (`:344-348`) now says its composition is written out in full where generations 2, 3 and 4 reach the same merge through `_append_generation`, keeping the prefix-equality sentence unchanged. The card's Logic already stated exactly that (the idiom is **eight instances**, seven of them one-line calls to the shared helper, and `_compose_generation_5` still spells its own composition out field by field), and the measurement confirms it: `_compose_generation_2/3/4/6/7/8/9` are each 9 lines with one non-docstring statement and `_compose_generation_5` is 17 lines with two. The registry, the dispatch, the pins and every declared generation are untouched by the diff (12 added / 9 removed comment lines, no statement changed), so no claim in this card is false. No row, citation or range was rewritten and no verification stamp advanced (the source is uncommitted and closeout owns the stamp).
