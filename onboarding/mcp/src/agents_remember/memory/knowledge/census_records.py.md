...
| The generation that registers these tables, bound to the registry's newest entry rather than to a literal number. | `REQUIRED_CENSUS_GENERATION`; `CURRENT_GENERATION` | mcp/src/agents_remember/memory/knowledge/census_records.py:118-120; mcp/src/agents_remember/memory/knowledge/schema_generations.py:476-479 |
...
| The schema tuples the decoders check against, declared beside the DDL they render into so the model and the schema cannot become two different sets of admitted values, and imported by the migration parser and inventory as the vocabularies their own closed sets must be subsets of. | `CENSUS_PARSE_OUTCOMES`; `CENSUS_DISPOSITION_KINDS`; `CENSUS_ARTIFACT_KINDS`; `CENSUS_INVENTORY_STATES` | mcp/src/agents_remember/memory/knowledge/schema_v9.py:73-115; mcp/src/agents_remember/memory/migration/parse.py:45-52; mcp/src/agents_remember/memory/migration/parse.py:112-113; mcp/src/agents_remember/memory/migration/inventory.py:115-119 |
