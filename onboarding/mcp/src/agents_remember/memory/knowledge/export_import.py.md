...
| **The node that proves an artifact whose sealed payload contradicts its digest is refused before publish, over both revision tables.** | "test_an_artifact_whose_sealed_payload_contradicts_its_digest_is_refused" | mcp/tests/test_knowledge_portable_boundaries.py:611-611 |
| **The node that proves destination admission refuses before any staging work, with every state's own code.** | "test_destination_admission_refuses_before_any_staging_work" | mcp/tests/test_knowledge_portable_boundaries.py:700-700 |
