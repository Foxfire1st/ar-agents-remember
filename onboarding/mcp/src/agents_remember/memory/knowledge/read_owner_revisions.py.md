...
| **The boundary case that proves a document present on disk at its recorded path still reports unavailable when the object store cannot produce the recorded revision.** | `test_an_owner_revision_the_object_store_cannot_obtain_is_reported_as_unavailable` | mcp/tests/test_knowledge_citation_boundaries.py:377-423 |
| The unit case that asserts this module reports only declared states. | `test_the_owner_revision_observation_reports_only_declared_states` | mcp/tests/test_knowledge_citation_bindings.py:246-255 |
