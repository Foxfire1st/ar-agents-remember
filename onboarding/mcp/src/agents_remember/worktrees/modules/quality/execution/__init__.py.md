...
Names the package for selected certification suffix authority and retained report transport.

## Code Commentary

### Logic

The file contains only its package docstring. It imports, exports and executes no adapter or initialization logic.

### Conventions

Import concrete owners directly: `models` for selected execution validation, `retained_reports` for bounded original bytes and `sandbox` for candidate admission/manifest composition.

### Invariants And Boundaries

- Importing the namespace does not select an operation, admit a runtime or start a gate.
- The package marker is not a compatibility facade or a second evidence store.
