...
| --- | --- | --- |
| The module docstring states the automatic-cleanup ruling and the report-never-raise boundary. | "The developer ruling is that a completed leaf is reclaimed without another prompt"; "A refused or failed cleanup is reported, never raised." | mcp/src/agents_remember/worktrees/modules/automatic_cleanup.py:1-14 |
...
| The caller that runs this after `integration_status=completed` and reloads the post-cleanup status payload. | `_integrated_result` | mcp/src/agents_remember/worktrees/modules/integrate.py:369-400 |
...
## Update History
- 2026-09-11T23:05:00+00:00: The docstring row used the bare phrase `module docstring` as its anchor, which the anchor grammar cannot read (a backticked span that is neither an identifier, a heading, nor a quoted literal), so the claim had no anchor at all. It now quotes two exact docstring sentences — the automatic-cleanup ruling and the report-never-raise boundary — whose text occurs once each inside the cited `automatic_cleanup.py:1-14`; claim wording and extent unchanged.
