...
| Focused integration tests exercise fast-forward sync and contract advance, a retained code merge conflict with continuation, stale-cache source-ref selection, and content-conflict preservation, including the knowledge conflict whose only working route is the manual continuation. | `test_pure_fast_forward_sync_advances_both_sides_and_contract`; `test_code_merge_conflict_is_retained_and_can_continue`; `test_sync_uses_source_refs_when_the_cache_is_stale_missing_or_malformed`; `test_memory_merge_settles_content_and_knowledge_conflicts_in_the_transaction` | mcp/tests/test_worktree_sync.py:494-514; mcp/tests/test_worktree_sync.py:516-551; mcp/tests/test_worktree_sync.py:553-579; mcp/tests/test_worktree_sync.py:698-719 |
...
## Update History
- 2026-09-20T07:45+02:00 — 260915-KS-L42 curator (citation repair in this document; this card's own source file is unchanged): **the row naming the focused integration cases was re-cited to those cases' own extents in the working tree.** This leaf's two new module-level helpers in `mcp/tests/test_worktree_sync.py` pushed every case in the `WorktreeSyncTests` class down the file, so the four ranges the row cited (`:420-441`, `:442-478`, `:479-506`, `:624-646`) no longer held their cases; they now read `:494-514`, `:516-551`, `:553-579` and `:698-719`. The Finding text gained the one fact this change set adds to that population — the knowledge conflict whose only working route is the manual continuation — because the same case now asserts it. No anchor was renamed, no citation dropped and no claim weakened; this card's metadata was not touched and no verification stamp was advanced.
