...
- `start.py`, `closeout.py`, `integrate.py`, `cleanup.py`, `finalize.py`, and `abandon.py`
  ...
  contract-owned repository/branch snapshot. `integrate.py` refuses while an undecided or policy-invalid `master-handover-approval` gate exists on the contract's lifecycle (the master-exit seam consumer, mirroring the closeout gate; gateless stays additive), then performs the code and
