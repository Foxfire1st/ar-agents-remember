...
| Whether a series still owns its integration line, read from the two progress cells a completion writes rather than from `cleanup`. | `_series_in_flight` | mcp/src/agents_remember/worktrees/reopen.py:824-833 |
...
| A series is live but unaddressed only when the locator at its own address is `terminal-archived` and both progress cells are untouched. | `_series_is_live_unaddressed` | mcp/src/agents_remember/worktrees/reopen.py:836-860 |
...
| The reopen clears the review counter a completion spent, and only when the counter carries history. | `_review_state_carries_history`; `_plan_series_document_reset` | mcp/src/agents_remember/worktrees/reopen.py:863-883; mcp/src/agents_remember/worktrees/reopen.py:886-923 |
...
| The publication `mode` (`reset` or `publish`) is decided by the arrival, not by a caller flag. | `_series_reopen_plan` | mcp/src/agents_remember/worktrees/reopen.py:926-981 |
...
- 2026-09-20T14:20+02:00 — 260915-KS-L43 curator (deterministic-check clearance inside this leaf's change set, uncommitted on `ar/260915-ks-l43-ar`, code base `fb719f89`): **`ruff format` normalisation only; no behaviour, no claim wording and no anchor changed.** Four spans were reflowed by the repository's own formatter: the `documentReset` mapping inside `_SeriesReopenPlan.payload()` (the dict literal split across lines), the `_series_ref_recut(...)` call in `_series_reopen_plan`, the `tombstone` conditional expression in the same function, and the `projectionEffects` list comprehension in `reopen_series`. Each edit sits at or after its enclosing construct's declaration, so **every construct below the first reflow moved down**: `_SeriesReopenPlan` `:733-758`→`:733-760`, `_series_reopen_plan` `:926-978`→`:926-981`, `reopen_series` `:1133-1207`→`:1133-1210`, and every construct from `_SeriesReopenPlan` onward with them. The card's ranges were re-measured for that reason and not because a claim changed: `_reopen_preflight_refusal` `:325-398`→`:325-397`, `_reopened_contract` `:197-224`→`:197-223`, `_ReopenPublication` `:519-579`→`:519-577`, `_series_in_flight` `:821-830`→`:824-833`, `_series_is_live_unaddressed` `:833-857`→`:836-860`, `_series_ref_recut` `:784-818`→`:787-821`, the counter row to `:863-883`/`:886-923`, `_series_reopen_plan` `:923-980`→`:926-981`, and the historical mention ranges in the history below to `:480-501`/`:580-585`/`:630-643`/`:646-702`. Every claim on this card was re-read against the reformatted source and **retained**: the formatter moves whitespace and nothing else, so the preflight, the identity-preserving contract reset, the two-output provenance clearing, the in-flight reading, the unaddressed-series test, the advance-never-move rule, the counter reset and the arrival-decided publication mode all still read correctly as written. **Stamp accounting:** `reviewedWorkingCandidate` now names this leaf's candidate `ar/260915-ks-l43-ar` on base `fb719f89`; the `lastVerifiedCommitHash`/`lastVerifiedCommitDate` pair is retained exactly as recorded, because no commit contains the body as it now stands and no stamp was measured on it. No commit was made.
