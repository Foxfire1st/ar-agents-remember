...
| lastUpdated            | 2026-06-24T06:26+02:00                                  |
...
finalization one shared way to find active root tasks, nested task roots, leaf `enclosures/<leaf-id>/`
contracts, and completed root-task archive targets.
...
leaf contract path or auto-selects the only leaf contract. Multiple leaves without an explicit `leaf_id`
raise `TaskResolutionError`, forcing callers to disambiguate without asking users for filesystem paths.
