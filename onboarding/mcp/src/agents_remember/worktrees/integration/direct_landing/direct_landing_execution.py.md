...
## 260821-CLIVE No Local Policy Delta

The CLIVE change is a signature adaptation only: execution now passes the configured contract into
`reconcile_direct_landing`. Recovery classification and policy remain owned by
`direct_landing_operation.py` and `direct_landing_recovery_state.py`; this executor gains no
fallback, authority, or lifecycle rule of its own.

## Update History

- 2026-08-24T14:43+02:00 — 260821-CLIVE cumulative curation: verified the contract-argument adaptation as no local policy impact. Timestamp is the curator host's Europe/Berlin system time; verification remains closeout-owned.
