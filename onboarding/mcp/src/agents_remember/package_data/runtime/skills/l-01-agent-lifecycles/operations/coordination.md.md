...
| path                   | `mcp/src/agents_remember/package_data/runtime/skills/l-01-agent-lifecycles/operations/coordination.md` |
...
Driving a portfolio or a master-series from durable state — routing backend events, recomputing the ready frontier, releasing work in order, and keeping the developer relay to one decision at a time.
...
The block carries only what a coordinating seat needs that its role file does not already own: the
orchestrator drives the portfolio, the manager drives exactly one master's leaf sequence, and the
architect wears either hat only in a solo/flat run. `## Required inputs` requires durable state rather
than the transcript and names the mechanical facts the control plane supplies (graph validity, derived
waves, completed predecessors, current closeout-door generations, closeout-projection validity, lineage,
gates, changed routes/seams). It also states that a manager must be given the master's explicit
`executionNature` and its sprint graph reference, and that a manager publishes door facts but never ranks
the portfolio.

### Conventions

Each owning seat's own series loop stays in its role file; this block does not become a second home for it.
...
| The canonical source of this block. | `# Operation` | skills/l-01-agent-lifecycles/operations/coordination.md:1-1 |
...
- 2026-09-16T08:01+02:00 — 260915-CAPS-L1 curator: created this card for `skills/l-01-agent-lifecycles/operations/coordination.md` — a file added by the role-instruction corpus consolidation. The canonical source is an operation-scoped procedure block extracted from interleaved role prose (coordination).; the packaged copy is produced by `scripts/sync-skills.py` and is not hand-edited. Verification metadata is left at the leaf base commit because the source is uncommitted — the governed closeout stamps the real code commit, and no hash or fingerprint was invented here.
