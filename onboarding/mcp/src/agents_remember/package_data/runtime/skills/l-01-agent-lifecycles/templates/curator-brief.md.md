...
The brief structurally dispatches a curator on the canonical leaf document without exposing a
runtime address. It requires three-way reconciliation before writing, routes current intent to the
...
- The brief cannot carry runtime, lifecycle, inbox-row, or gate ids as work addresses.
- Curator writes memory only and cannot stamp, close out, integrate, or decide gates.
...
The curator brief carries the exact frozen candidate, passing per-route verdict references, current
