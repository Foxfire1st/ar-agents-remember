...
orchestrator in a flat series) compiles a worker brief from. The brief is the worker's **entire
...
doctrine: the frictions it absorbs are route-index leaks into the official checkout (F-E), the
literal no-route-impact attestation form (F-F), provider-stack keying confusion (F-H), and the
missing `python` shim in spawn environments (F-I).
...
brief with `<placeholder>` slots: leaf identity, the two writable worktrees (code + memory) with
...
shim), the task statement, the checks ladder (focused + full wrapper + `git diff --check`), the
same-pass c-05 onboarding rules including the literal `- <ISO timestamp> — No route impact:
<reason>` attestation and the pin idiom, and the mandatory turn-report path. Compiler notes bind
the spawning seat: fill every placeholder (an unresolved placeholder is not dispatchable), verify
