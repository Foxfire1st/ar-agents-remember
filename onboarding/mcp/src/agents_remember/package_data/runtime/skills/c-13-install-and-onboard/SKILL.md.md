...
After the developer copies a harness starter package, wires the MCP server, and
restarts the harness once, this skill verifies/runs `runtime_install`, sets up or
adopts memory, bootstraps onboarding when needed, and configures providers.
...
the starter package's responsibility. Stage 0 preflight checks that the MCP is
reachable, the harness package appears present, settings paths are sane, runtime
state is known, provider prerequisites are understood when providers are enabled,
and memory topology is consistent. It does not check hook prerequisites such as
`jq`, because this skill no longer installs hooks or instruction files.
...
| Finding | Citations | Source Path |
| --- | --- | --- |
| The skill starts only after the harness package is copied, MCP is wired, and the harness has restarted once; package files own skills, hooks, rules, instructions, MCP templates, and settings templates. | L8-L22 | [`c-13-install-and-onboard` SKILL.md](agents-remember-md/mcp/src/agents_remember/package_data/runtime/skills/c-13-install-and-onboard/SKILL.md) |
