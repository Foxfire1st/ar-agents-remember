...
`test_a_consumed_approval_cannot_be_replayed_by_a_second_closeout` cit:([`test_a_consumed_approval_cannot_be_replayed_by_a_second_closeout`], mcp/tests/test_gate_replay_window.py:222-235) is the contract
...
`test_the_applied_record_is_the_only_thing_closing_the_window` cit:([`test_the_applied_record_is_the_only_thing_closing_the_window`], mcp/tests/test_gate_replay_window.py:237-263) is the
...
`test_the_applied_record_survives_a_concurrent_gate_log_compaction` cit:([`test_the_applied_record_survives_a_concurrent_gate_log_compaction`], mcp/tests/test_gate_replay_window.py:265-294) is the regression.
Two real forked processes: cit:([`_compactor_main`], mcp/tests/test_gate_replay_window.py:151-156) appends a prunable `alarm-ack` gate so
...
cit:([`_consumer_main`], mcp/tests/test_gate_replay_window.py:142-148) waits for the park and drives the real `_enforce_closeout_gate` +
...
`test_the_gate_fold_refuses_rather_than_skipping_a_torn_approval_record` cit:([`test_the_gate_fold_refuses_rather_than_skipping_a_torn_approval_record`], mcp/tests/test_gate_replay_window.py:296-328) appends an
...
**The fixture is where the honesty is.** cit:([`_seed_approved_gate`], mcp/tests/test_gate_replay_window.py:83-117) builds a real
...
**`_prunable_gate` exists so a compaction has something to do** cit:([`_prunable_gate`], mcp/tests/test_gate_replay_window.py:120-134). Every store in this
...
**`test_the_gate_fold_refuses_rather_than_skipping_a_torn_approval_record` cit:([`test_the_gate_fold_refuses_rather_than_skipping_a_torn_approval_record`], mcp/tests/test_gate_replay_window.py:296-328) asserts
...
| The pure snapshot the append carries: same gate id, `state="applied"`, decision attribution carried forward unchanged. | `apply_gate` | mcp/src/agents_remember/controlplane/records.py:189-200 |
...
| Why the append now survives: the lock is unconditional across append and rewrite, and the rewrite never unlinks. | `exclusive_access`; `rewrite_lines` | mcp/src/agents_remember/controlplane/durable_store.py:393-448; mcp/src/agents_remember/controlplane/durable_store.py:509-516 |
...
## 260821-CLIVE-L1 Evidence Migration
...
- 2026-08-01T14:20+02:00 — 260731-EFA-L5 curator: created the card for the replay-window suite.
  ...
  cit:(["def apply_gate"], mcp/src/agents_remember/controlplane/records.py:189-189) appending `apply_gate` cit:([`apply_gate`], mcp/src/agents_remember/controlplane/records.py:189-200), refused on the next
  ...
  cit:([`test_the_gate_fold_refuses_rather_than_skipping_a_torn_approval_record`], mcp/tests/test_gate_replay_window.py:296-328) asserts
