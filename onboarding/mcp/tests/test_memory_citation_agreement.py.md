...
| The `T52` REPORT-ONLY population pin on the master memory branch at the tip this leaf lands, which is what `260918-TSIP-L11` re-derived it to: **123** rows. It is a report and not a gate, and the rows that moved it are named at the constant. | `T52_DEFINITION_OUTSIDE_RANGE` | mcp/tests/test_memory_citation_agreement.py:114-114 |
| The ENFORCED population on the same tree, so the counter and the row list must agree: **0** on the landed pair, which is the assertion rather than an empty pin, and a future nonzero reading means the enforced population regrew. | `T52_ENFORCED_POPULATION` | mcp/tests/test_memory_citation_agreement.py:115-115 |
| The `T58` pins, derived by enumeration rather than from the fixer's counter. | `T58_WRAPPED_CITATIONS` | mcp/tests/test_memory_citation_agreement.py:116-117 |
