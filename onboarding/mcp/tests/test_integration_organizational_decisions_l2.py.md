# mcp/tests/test_integration_organizational_decisions_l2.py

| Field | Value |
| --- | --- |
| repository | agents-remember |
| path | `mcp/tests/test_integration_organizational_decisions_l2.py` |
| doc_type | `file-level-onboarding` |
| lastUpdated | 2026-08-24T14:48+02:00 |
| lastVerifiedCommitHash | `f95487ec993b58d34911bba0206a7fa6ef9684eb` |
| lastVerifiedCommitDate | 2026-08-24T15:28:18+02:00|
| governingOverview | `overview.md` |

## Governing Overview

[mcp tests overview](overview.md)

## Purpose

Forces integration organizational decisions l2 behavior at the public and durable-state boundaries.

## Code Commentary

### Logic

The principal forcing seams are `test_third_task_bytes_make_fresh_status_and_stale_recover_identical`, `test_task_change_after_public_preflight_stops_protected_publication`, `test_unreadable_organizational_json_is_one_bounded_public_decision`, `test_unreadable_organizational_markdown_is_one_bounded_public_decision`. The suite forces claim transfer, door authority, exact protected-ref classification, publication recovery, and same-generation reconciliation without moving lifecycle evidence into the queue.

### Conventions

Tests address operations by task/contract plus kind and generation, assert durable evidence and public legal controls, and compare state across failure cuts. Helpers remain test-only and invoke the same public/domain seams as production.

### Invariants And Boundaries

- A passing assertion must prove the advertised action executes or terminates safely; payload shape alone is insufficient.
- Queue projection is never accepted as lifecycle evidence, and private operation identifiers do not cross the public test boundary.
- Failure-path assertions check non-mutation or exact same-generation recovery, not merely an exception string.

### Todos

None recorded.

## Docs References

No configured Domain Documentation source applies to these repository-internal forcing tests.

## Repo-Internal References

The test source is the direct evidence for the regression contract.

| Finding | Citations | Source Path |
| --- | --- | --- |
| The file defines `test_third_task_bytes_make_fresh_status_and_stale_recover_identical`, `test_task_change_after_public_preflight_stops_protected_publication`, `test_unreadable_organizational_json_is_one_bounded_public_decision`, `test_unreadable_organizational_markdown_is_one_bounded_public_decision` as its principal forcing seams. | L135-L176; L178-L228; L293-L294; L296-L299 | `mcp/tests/test_integration_organizational_decisions_l2.py` |

## Cross-Repo References

No meaningful cross-repository boundary is owned by this test file.

## Current Contract — 260821 CLIVE Final

This is the current source-backed contract for this test card. It supersedes any earlier
queue-lifecycle, blocker-row, replan/drain, or compatibility-reader wording where present.

Forces public totality for journal-owned organizational task publication decisions and retries.

### Current Invariants

- Organizational publication decisions are durable operation evidence.
- Refusal or retry remains task-addressed and does not depend on queue artifacts.

## Update History

- 2026-08-24T14:48+02:00 — DAGQC cumulative CLIVE final-gap curation: reconciled this test card to current source while preserving prior history and verification provenance.

- 2026-08-23T16:08+02:00 — 260821-CLIVE-L2: created from the accepted full L2 candidate. Verification fields remain blank until the architect-owned closeout has a real code commit to stamp.
