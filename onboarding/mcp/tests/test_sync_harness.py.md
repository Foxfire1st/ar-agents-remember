...
| Both hook tiers run the same `--check`. | "scripts/sync-harness.py" | .githooks/_gate.sh:107-107 |
...
- 2026-08-28T15:52:15+02:00 — Rebased the `_gate.sh` citation after the MCP-local interpreter
  selection repair shifted the generated-harness check; test ownership and behavior are unchanged.
