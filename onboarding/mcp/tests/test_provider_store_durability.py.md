...
test time and proving the same instrument still detects loss there (`ProviderHarnessSensitivityTests`,
L348-L382). Without that class the zero is an unfalsifiable green; with it, the zero is a
...
**`ProviderStoreDurabilityTests` (L275-L345) — R10, three tests over real processes.**
`test_no_record_is_lost_when_an_append_races_a_compaction` (L278-L289) drives the deterministic
...
`test_no_record_is_lost_under_sustained_multi_process_write_and_compaction` (L291-L313) is the
...
`test_concurrent_operation_never_raises_out_of_a_store_call` (L315-L345) is deliberately separate
...
**`ProviderHarnessSensitivityTests` (L348-L382) — R14, and the property that makes the zero mean
anything.** `test_the_forced_scenario_detects_loss_in_the_base_commit` (L356-L382) calls
...
**`ProviderReadPolicyTests` (L385-L562) — R8, why TOLERANT is the right answer for both logs.**
...
are now asserted** — the class docstring (L386-L408) claims that standard of evidence and now meets
...
1. **Neither rewrite parses.** `metrics.compact` reclaims from a raw byte tail (`_tail_lines`) and
   ...
   `test_neither_reclaim_deletes_a_row_it_could_not_parse` (L474-L502) is the assertion that holds
   this: it appends `TORN_LINE` (L111) to each log, runs each reclaim, and requires the torn line to
...
2. **Nothing is decided on the presence of a row**, in two halves with a test each.
   `test_the_metrics_consumer_writes_nothing_back_to_the_log_it_reads` (L504-L539) runs the real
   ...
   **bytes** before and after, so "nothing marks a row spent" is measured rather than asserted about.
   ...
   `test_nothing_outside_the_degradation_module_reads_the_event_log` (L541-L562) covers the other
...
The remaining three tests pin the tolerance itself: `test_the_metrics_read_skips_only_the_row_it_
cannot_read` (L421-L442) interleaves a torn line, a non-object (`[1, 2, 3]`), a
`FUTURE_MAJOR_ROW` (L115-L121) and whitespace between two real samples and requires the read to
return **both** real samples — per row, not per file, so an intact row written *after* a bad one
still survives; `test_a_row_with_no_schema_version_is_read_as_1_0` (L444-L458) proves the stamping
is additive rather than a migration, on `metrics.jsonl` and `metrics-current.json` alike; and
`test_read_current_reports_nothing_it_cannot_interpret` (L460-L472) requires a newer-major
current-state file to read as "no current sample" rather than be handed to the status packet as
though it were understood.

**`ProviderSchemaVersionTests` (L565-L618).** `test_every_row_this_build_writes_carries_its_
schema_version` (L568-L592) checks all three writes that produce a record — `record`,
`record_index_state`, `append_event` — plus `metrics-current.json`. Its counterpart,
`test_the_state_document_carries_no_schema_version` (L594-L618), asserts the *absence* of the
...
**`ProviderOwnershipTests` (L621-L708) — R2, the ownership decisions as assertions rather than as
prose in a rationale string.** `test_both_provider_logs_declare_the_dashboard_their_compaction_owner`
(L630-L647) requires `compaction_owner == "dashboard"` on both `StoreOwnership` declarations and
...
`test_each_provider_reclaim_has_exactly_one_call_site` (L649-L674) is what closes that: it asserts
`provider_reclaim_call_sites()` (L160-L189) equals exactly
...
tree that have one is not miscounted, and `_own_scope` (L124-L133) refuses to descend into nested
functions so a reference is never attributed to two enclosing scopes.
`test_the_metrics_log_accepts_a_write_from_either_daemon` (L676-L688) is the one that encodes the
...
`test_the_degradation_log_refuses_a_write_from_the_mcp_process` (L690-L708) is the check that can
actually fire in this pair — `writers=("dashboard",)` — and it covers both of that store's writes,
the append and the state document.

**`ProviderReclaimShapeTests` (L711-L786).** The reclaim's edges, including the failure mode this
pair never had: `test_a_reclaim_with_no_log_yet_is_a_no_op` (L714-L716) over both stores,
`test_no_reclaim_on_either_log_ever_unlinks_it` (L718-L774), and
`test_the_reclaim_keeps_the_newest_rows_and_returns_how_many` (L776-L786), which pins that
...
**`ProviderCaseRegistryTests` (L789-L798).** One test, and it is a boundary rather than a
...
(`_store_durability.py` L401-L522), which is what lets them reuse the scenarios, the raw on-disk
accounting and the base-commit archive. But `CASES` still enumerates only the six control-plane
adapters and `PROVIDER_CASES` only the two provider ones (L540-L544) — so widening the instrument
...
docstring (L1-L59) carries several of them and then disclaims them in the same breath: an early
...
**One throwaway store tree per (scenario, case).** `_TempRootTest.case_root` (L258-L272) returns
...
worked around here, then fixed at the source: `harness_work_dir` (L790-L817) now derives the
scratch directory from `root` itself, and `MIN_RECLAIM_TICKS` (L826-L841) refuses a result whose
...
`reclaim_ticks > 1` moved into the instrument as `MIN_RECLAIM_TICKS = 10`, so it now applies to
every caller of `run_stress` — including the control-plane suite that did not have it and the
base-commit script entry point that could not have had it. `_describe` (L240-L249) carries the tick
...
- This suite spawns real processes and runs a stress profile per store; it is not a unit test, and
  ...
  budget.
- `ProviderOwnershipTests` calls `declare_process_role` directly. It is safe to do so only because
  of the autouse `restore_declared_process_role` fixture in `conftest.py`; without it the three of
  its four tests that declare a role would stamp it onto every later test in the same interpreter.
...
1. *`ProviderReadPolicyTests` claimed both structural facts were asserted and only fact 1 was.*
   ...
   `test_the_metrics_consumer_writes_nothing_back_to_the_log_it_reads` (L504-L539) and
   `test_nothing_outside_the_degradation_module_reads_the_event_log` (L541-L562) — and the class
   docstring (L386-L408) says so in the same words it used to argue in.
2. *`test_the_dashboard_compacts_both_provider_logs` was named for a claim it did not make.* Renamed
   `test_both_provider_logs_declare_the_dashboard_their_compaction_owner` (L630-L647), which is what
   it actually proves, and the claim it did not make is now made by
   `test_each_provider_reclaim_has_exactly_one_call_site` (L649-L674). The second `compact` call site
...
3. *`test_a_reclaim_that_keeps_nothing_leaves_the_log_in_place` drove one store through one path.*
   Renamed `test_no_reclaim_on_either_log_ever_unlinks_it` (L718-L774) and widened to both stores
...
4. *The module docstring enumerated "four things pinned here" while the file had seven classes.* It
   now reads "Seven classes pin seven things" and enumerates all seven (L29-L51), stating outright
...
| Finding | Citations | Source Path |
| --- | --- | --- |
| The metrics store under test: the tolerant per-row read and its escalation clause, the reclaim that holds one lock across stat + tail + rewrite and drops rows by age only, the two appends that make the pairing two-process, the lock-free tail read, and the ownership declaration. | `_parse_row` L191-L228; `compact` L302-L341; `record` L254-L267; `record_index_state` L269-L283; `read_recent` L343-L360; `PROVIDER_METRICS_OWNERSHIP` L65-L83 | [providers/metrics.py](agents-remember/mcp/src/agents_remember/providers/metrics.py) |
| The degradation store under test: the single-writer ownership declaration whose `check_declared_writer` can actually fire, the count-based reclaim under one held lock, the stamped append, and the state document that is deliberately unversioned. | `PROVIDER_DEGRADATION_OWNERSHIP` L85-L104; `write_state` L191-L216; `append_event` L218-L232; `compact_events` L234-L254 | [providers/degradation.py](agents-remember/mcp/src/agents_remember/providers/degradation.py) |
| The measurement instrument this file imports: the two provider adapters and the root-resolution they override, the deliberately disjoint case lists, the per-run work directory, the reclaim-tick floor, the shared stress profile, and the base-commit archive plus its pinned re-execution. | L401-L522; L540-L544; `harness_work_dir` L790-L817; `MIN_RECLAIM_TICKS` L826-L841; `STRESS_PROFILE` L1040-L1047; `extract_base_commit_tree` L1069-L1093; `run_against_source` L1096-L1118 | [_store_durability.py](agents-remember/mcp/tests/_store_durability.py) |
| The contract both stores were moved onto: the unconditional per-log lock, the never-unlinking rewrite, the version policy the reads implement, and the role declaration the ownership tests drive. | `exclusive_access`; `append_line`; `rewrite_lines`; `SCHEMA_VERSION`; `schema_version_supported`; `StoreOwnership`; `declare_process_role`; `CompactionOwnerError` | [controlplane/durable_store.py](agents-remember/mcp/src/agents_remember/controlplane/durable_store.py) |
| The two-process pairing asserted by `ProviderOwnershipTests`, as it actually runs: the dashboard loop that samples, records, evaluates degradation and then compacts. It is also the metrics reclaim's one call site, and it reaches it as `await asyncio.to_thread(metrics_store.compact)` — a reference rather than a call, which is why `provider_reclaim_call_sites` counts references. | `_metrics_loop` L806-L818 | [serving/app.py](agents-remember/mcp/src/agents_remember/serving/app.py) |
| The other half of that pairing: the MCP process's provider-setup thread appending index-lifecycle rows into the same log. | `_record_index_state` L434-L453 | [providers/provider_setup.py](agents-remember/mcp/src/agents_remember/providers/provider_setup.py) |
| The consumer that makes the metrics log's tolerant read structurally safe: the whole state machine is re-derived from a rolling window of live samples and nothing is consumed. | `evaluate_provider_degradation` L260-L311 | [providers/degradation.py](agents-remember/mcp/src/agents_remember/providers/degradation.py) |
| The control-plane suite over the same instrument, whose `CASES` this file's `PROVIDER_CASES` is asserted to be disjoint from, and whose base-commit class this file's mirrors. | `MultiProcessDurabilityTests` L119-L201; `HarnessVacuityGuardTests` L335-L377; `HarnessSensitivityTests` L380-L435 | [test_controlplane_store_durability.py](agents-remember/mcp/tests/test_controlplane_store_durability.py) |
| The autouse fixture that makes `ProviderOwnershipTests`' direct `declare_process_role` calls safe for every test that runs after them. | `restore_declared_process_role` L57-L87 | [conftest.py](agents-remember/mcp/tests/conftest.py) |
...
- 2026-08-01T20:45+02:00 — 260731-EFA-L5 worker: **repointed the card onto the suite as it now
  ...
  `test_both_provider_logs_declare_the_dashboard_their_compaction_owner` (L630-L647), joined by a new
  `test_each_provider_reclaim_has_exactly_one_call_site` (L649-L674). That new test is the one that
  matters: `provider_reclaim_call_sites` (L160-L189) is an **AST scan that resolves receivers bound
  ...
  nothing and report the property held. So the "exactly one caller, inside the dashboard's loop"
  ...
  `test_no_reclaim_on_either_log_ever_unlinks_it` (L718-L774), now driving **both stores through both
  ...
  `test_the_metrics_consumer_writes_nothing_back_to_the_log_it_reads` (L504-L539) and
  `test_nothing_outside_the_degradation_module_reads_the_event_log` (L541-L562), which assert what
  ...
  a file of seven classes but enumerates all seven (L29-L51). The Todos section now records the
...
- 2026-08-01T16:15+02:00 — 260731-EFA-L5 curator: created the card for the provider-store
  ...
  `ProviderHarnessSensitivityTests` (L203-L237): it `git archive`s the leaf's base commit **at test
  ...
  disjoint set (`_store_durability.py` L525-L545), so widening the instrument cannot silently widen
  what `test_controlplane_store_durability.py` asserts — and `ProviderCaseRegistryTests` (L499-L508)
