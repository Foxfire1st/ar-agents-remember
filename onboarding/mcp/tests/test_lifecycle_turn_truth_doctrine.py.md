...
| Finding | Anchor | Source |
| --- | --- | --- |
| The doctrine's one home, which the census and the shared-root case follow. | `# Core — Completion Truth And Handoff Acceptance` | skills/l-01-agent-lifecycles/core/acceptance.md |
| The declared roster and the vocabulary whose speakers it must equal, in both directions. | `COMPLETION_TRUTH_ROSTER`; `COMPLETION_TRUTH_VOCABULARY` | mcp/tests/test_lifecycle_turn_truth_doctrine.py:112-131 |
...
| The contradiction sweep and its three-state reading classification. | `contradictions`; `classify_reading`; `RETIRED_CLAIMS` | mcp/tests/test_lifecycle_turn_truth_doctrine.py:551-564; mcp/tests/test_lifecycle_turn_truth_doctrine.py:585-597; mcp/tests/test_lifecycle_turn_truth_doctrine.py:388-464 |
