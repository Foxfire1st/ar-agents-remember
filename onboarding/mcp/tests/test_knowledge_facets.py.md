...
| Finding | Anchor | Source |
| --- | --- | --- |
| **The closure case: the payload union, the schema map, the seam's kind groups and the derived envelope row — re-scoped by `KS-R14@v1`, again for the census group by `260915-KS-L21` (`set(CENSUS_RECORD_KINDS)` at `:291`, inside the range below), and the case that carries the `RE-SCOPED` paragraph.** | "test_the_seam_registry_is_exactly_the_eight_declared_subtypes" | mcp/tests/test_knowledge_facets.py:183-247; mcp/tests/test_knowledge_facets.py:228-306 |
| **The three census vocabulary constants this leaf's two re-scoped cases import — the group's record kinds, command kinds and writable tables — added to the explicit import block at the top of the module.** | `CENSUS_RECORD_KINDS`; `CENSUS_COMMAND_KINDS`; `CENSUS_WRITABLE_TABLES` | mcp/tests/test_knowledge_facets.py:80-83 |
| **The per-subtype walk: a valid shape, an undeclared field, a dropped meaning, three provenance substitutions and the ninth subtype.** | "test_every_subtype_refuses_a_bad_shape_a_missing_meaning_and_its_own_provenance" | mcp/tests/test_knowledge_facets.py:344-344 |
| The ninth subtype refused through the write path with the table counts unchanged. | "test_an_unknown_ninth_subtype_is_refused_and_never_stored_as_a_generic_facet" | mcp/tests/test_knowledge_facets.py:404-404 |
| **The receipt's field set, the seven absent verdict words and the stored authorship-versus-decider split.** | "test_a_facets_authorship_lifecycle_and_receipt_are_stored_data_with_no_verdict" | mcp/tests/test_knowledge_facets.py:427-427 |
| **Accepted origin data refused at both entry points with the shipped code.** | "test_accepted_origin_data_is_refused_at_both_entry_points" | mcp/tests/test_knowledge_facets.py:467-467 |
| **The four endpoint kinds attached and read back, the checked group measured in DDL, and every kind's missing-target refusal.** | "test_every_endpoint_kind_is_a_checked_group_that_refuses_a_missing_target" | mcp/tests/test_knowledge_facets.py:491-491 |
| **The removal that names its row, refuses a stale digest and deletes that row only.** | "test_removing_an_attachment_names_its_row_and_deletes_that_row_only" | mcp/tests/test_knowledge_facets.py:576-576 |
| The ungoverned and unattached states as explicit values, and the envelope's route association. | "test_an_unattached_facet_and_the_envelopes_route_association_are_explicit_states" | mcp/tests/test_knowledge_facets.py:615-615 |
| **The three digests unchanged across a supersession, and the edge visible from the superseded side.** | "test_a_superseding_decision_authors_an_edge_and_the_superseded_one_is_retained" | mcp/tests/test_knowledge_facets.py:660-660 |
| **The cycle rollback, the sealed rows the database refuses to rewrite or delete, and the non-decision target refusal.** | "test_a_supersession_cycle_rolls_back_and_a_sealed_decision_cannot_be_rewritten" | mcp/tests/test_knowledge_facets.py:710-710 |
...
| **The widened union and the dispatch tables, now closed over every declared record group rather than over a count — `set(CENSUS_COMMAND_KINDS)` at `:953` and `set(CENSUS_WRITABLE_TABLES)` at `:981` are this leaf's two additions inside the range below, which is why the exact-total wording this row used to carry is superseded.** | "test_the_facet_commands_join_the_closed_union_and_its_dispatch_tables" | mcp/tests/test_knowledge_facets.py:913-982 |
| **Both entry points, the batch rollback, the wrong-operation refusal and the facet-row expectation.** | "test_the_two_entry_points_agree_and_a_refused_write_writes_nothing" | mcp/tests/test_knowledge_facets.py:985-985 |
| **The registry, the additive prefix, the `STRICT` idiom and the rebind trigger's named columns — re-scoped by `KS-R14@v1` so the created generation is named as the newest registered one.** | "test_the_registered_generation_appends_only_and_the_preceding_ones_are_unchanged" | mcp/tests/test_knowledge_facets.py:1065-1065 |
| **The dataset that predates the facet tables: refused, unmigrated and still served as its own generation.** | "test_a_dataset_predating_the_facet_tables_refuses_a_facet_write" | mcp/tests/test_knowledge_facets.py:1162-1162 |
| **The byte-identity comparison against the pre-leaf page and result digests, with the shipped constants unchanged.** | "test_the_shipped_seed_page_is_byte_identical_and_the_facet_page_is_its_own_policy" | mcp/tests/test_knowledge_facets.py:1214-1214 |
...
## Update History
- 2026-09-18T17:00+02:00 — 260915-KS-L21 curator (uncommitted change set on `ar/260915-ks-l21`, base `a7076008`): re-read this card's two shared-substrate claims and its population claim against the current source and corrected all three. The census leaf registers three record kinds plus the three relations they resolve through, so `test_the_seam_registry_is_exactly_the_eight_declared_subtypes` gained `set(CENSUS_RECORD_KINDS)` in its group tuple (`mcp/tests/test_knowledge_facets.py:291`) and `test_the_facet_commands_join_the_closed_union_and_its_dispatch_tables` gained `set(CENSUS_COMMAND_KINDS)` (`:953`) and `set(CENSUS_WRITABLE_TABLES)` (`:981`), all three imported from `agents_remember.models.knowledge.census` (`:80-83`); the card now records that the subset, disjointness and partition assertions cover the group as vocabulary rather than as literals, and that the facet half is untouched. **The budget claim was re-read and re-worded rather than carried:** the card's retained `KS-R14@v1` reading of 1315 unit / 322 integration against `unit_case_budget = 1500` / `integration_case_budget = 400` is kept as the state it measured, and the paragraph now states the pair the file declares now — **`unit_case_budget = 2300` (`pyproject.toml:244`)** and **`integration_case_budget = 400` (`pyproject.toml:245`)** — measured on this candidate at **2206** unit and **394** integration, because the census leaf's 48 unit cases took the merged population six past the then-declared 2200 and the unit ceiling was raised with the measured entry recorded above the value; no superseded value was deleted. Two rows were re-cited to ranges that contain their anchors and to the additions behind them. No case was deleted, skipped, deselected or weakened and this module still holds its 17 cases. The metadata block above now names this leaf's candidate as what was read and carries **no `lastVerifiedCommitHash`**: the body was re-read against a working candidate no commit contains, so no real commit holds the content a stamp would claim to have verified, and closeout owns the stamp. The body was changed substantively and this entry is the history record, not a metadata-only refresh.
