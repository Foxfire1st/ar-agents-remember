...
| The test suite this file joins and its fixture/collection conventions. | `## Fixture Roles And Claims`; `## Isolation And Collection` | onboarding/mcp/tests/overview.md:1021-1046 |
...
## Update History
- 2026-09-20T17:17:10+00:00: Generated citation repair: `## Fixture Roles And Claims`; `## Isolation And Collection` repointed to onboarding/mcp/tests/overview.md:1021-1042; onboarding/mcp/tests/overview.md:1043-1046. No content impact: mechanical anchor-range projection bound to citation source snapshot 02d8f0b256fe50bf7458ae56160b7e02e4466423e76d3ab197e12477f370f5b8; claim bytes unchanged; generated by ccr-r10@v1.
- 2026-09-20T05:38+02:00 — 260915-KS-L41 curator (cross-card pointer repair; uncommitted change set on `ar/260915-ks-l41-ar`, code base `756c47b3` before the L39 sync): **this card's one memory-tree row was re-pointed a third time, for the same reason as the two entries below and against the merged document rather than by arithmetic on the old numbers — no claim, no anchor and no body byte of this card changed.** `onboarding/mcp/tests/overview.md` now carries TWO sections above the two headings this row cites: 260915-KS-L39's and 260915-KS-L41's. L39's own reconciled cells (`:846-867`, `:868-871`) were measured before L41's section existed, so in the merged document `## Fixture Roles And Claims` is at `880` (section extent through the blank line before `902`) and `## Isolation And Collection` at `902` (extent through `905`). The cells were re-read against the merged file and now cite `880-901` and `902-905`; each is the heading's own section extent, which is the form every earlier repair on this row used, because a bare heading line does not carry the anchor once the checker strips the trailing newline. `mcp/tests/test_eve_protocol.py` is byte-identical to this leaf's base, so no verification stamp was advanced and none was invented — the retained pair is L39's landed `f79f4db745ad00b908d6ce4871d0b4ab2320207c`, and the `reviewedWorkingCandidate` row is left naming L39's candidate because this pass changed only a coordinate into another document.
...
- 2026-09-18T13:46+02:00 — 260918-TSIP-L2 curator (uncommitted change set on `ar/260918-tsip-l2-ar`,
  ...
  moved both headings by +43 — `## Fixture Roles And Claims` `646` → **`689`** and
  `## Isolation And Collection` `668` → **`711`**. The two ranges were re-derived from the **post-edit**
