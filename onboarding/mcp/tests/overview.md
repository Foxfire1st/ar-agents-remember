...
| The lane row that admits the L3 module: `test_memory_backfill.py` in the `unit-regression` lane (row 77 after the KS-L3 insertions). | "mcp/tests/test_memory_backfill.py" | mcp/tests/test-evidence-lanes.toml:95-95 |
...
| The L36 lane registration the fail-closed manifest requires (row 154 after the KS-L2 and KS-L3 insertions). | "mcp/tests/test_cross_master_concurrency.py" | mcp/tests/test-evidence-lanes.toml:190-190 |
...
| The lane registration the fail-closed manifest requires for that module (row 165 after the KS-L2 and KS-L3 insertions). | "mcp/tests/test_lifecycle_playthrough_end_to_end.py" | mcp/tests/test-evidence-lanes.toml:201-201 |
| The L4 census module's lane row, which closed the gap the L4 route section recorded (row 76 after the KS-L3 insertions). | "mcp/tests/test_memory_attribution_producers.py" | mcp/tests/test-evidence-lanes.toml:94-94 |
| The L5 binding module's lane registration, added by the same change set that created it (row 162 after the KS-L2 and KS-L3 insertions). | "mcp/tests/test_leaf_doc_master_link_binding.py" | mcp/tests/test-evidence-lanes.toml:198-198 |
| **260913-LCA-L8:** the new module's lane registration, added by the same change set that created it — integration, because it drives the public landing, integration and finalization routes over real temporary repositories and worktrees. | "mcp/tests/test_terminal_blocker_reasons.py" | mcp/tests/test-evidence-lanes.toml:223-223 |
...
| **260913-LCA-L8:** the nine exact-consumer rows the new module's change set adds to the evidence registry. | "mcp/tests/test_terminal_blocker_reasons.py" | mcp/tests/test-evidence-lanes.toml:223-223 |
| The lane row that admits the L3 module: `test_memory_backfill.py` in the `unit-regression` lane. | "mcp/tests/test_memory_backfill.py" | mcp/tests/test-evidence-lanes.toml:95-95 |
...
| The L36 lane registration the fail-closed manifest requires. | "mcp/tests/test_cross_master_concurrency.py" | mcp/tests/test-evidence-lanes.toml:190-190 |
...
| The lane registration the fail-closed manifest requires for that module. | "mcp/tests/test_lifecycle_playthrough_end_to_end.py" | mcp/tests/test-evidence-lanes.toml:201-201 |
| The L4 census module's lane row, which closed the gap the L4 route section recorded. | "mcp/tests/test_memory_attribution_producers.py" | mcp/tests/test-evidence-lanes.toml:94-94 |
| The L5 binding module's lane registration, added by the same change set that created it. | "mcp/tests/test_leaf_doc_master_link_binding.py" | mcp/tests/test-evidence-lanes.toml:198-198 |
...
| The L23 lane registration the fail-closed manifest requires, inserted between two alphabetically adjacent rows so no other entry moved. | "mcp/tests/test_terminal_liveness_registration_order.py" | mcp/tests/test-evidence-lanes.toml:152-152 |
...
| The architecture-fitness lane registration the fail-closed manifest requires for that proof. | "mcp/tests/test_terminal_liveness_pane_authority.py" | mcp/tests/test-evidence-lanes.toml:225-225 |
...
| Finding | Anchor | Source |
| --- | --- | --- |
| **The two new modules' lane rows, registered by the same change set that created them.** | "mcp/tests/test_knowledge_requirement_reference_contract.py"; "mcp/tests/test_knowledge_requirement_revisions.py" | mcp/tests/test-evidence-lanes.toml:81-81; mcp/tests/test-evidence-lanes.toml:82-82; mcp/tests/test-evidence-lanes.toml:84-84; mcp/tests/test-evidence-lanes.toml:83-83 |
...
- 2026-09-18T02:37:44+00:00: Generated citation repair: "mcp/tests/test_knowledge_read_boundaries.py"; "mcp/tests/test_knowledge_read_paths.py" repointed to mcp/tests/test-evidence-lanes.toml:164-164; mcp/tests/test-evidence-lanes.toml:166-166. No content impact: mechanical anchor-range projection bound to citation source snapshot d211cfd02f11c0600198b11c621aa5574ac8743db6e0ca1d2c92936e561c5146; claim bytes unchanged; generated by ccr-r10@v1.
...
| Finding | Anchor | Source |
| --- | --- | --- |
| The knowledge suite and its unit-lane placement (row 74 after the KS-L3 insertions). | "mcp/tests/test_knowledge_store.py" | mcp/tests/test-evidence-lanes.toml:89-89 |
...
| The label-operations suite's row and the reason it exists as a separate module. | "mcp/tests/test_knowledge_label_operations.py" | mcp/tests/test-evidence-lanes.toml:76-76 |
...
| **The lane row the unit half was registered in — the lane header itself, which is what an anchor naming a lane is satisfied by.** | "unit-regression" | mcp/tests/test-evidence-lanes.toml:5-5 |
| **The lane row the integration half was registered in, and the budget statement it sits under.** | "integration" | mcp/tests/test-evidence-lanes.toml:164-164 |
...
| The two lane rows this leaf registered, and the three consumer lists it extended. | "integration = [" | mcp/tests/test-evidence-lanes.toml:164-164; mcp/tests/evidence-lifecycle.toml:1031-1059; mcp/tests/evidence-lifecycle.toml:1110-1133; mcp/tests/evidence-lifecycle.toml:1135-1159 |
...
|The read fixture and its registered contract: the harness entry point, the artifact row, and the replacement contract that names it.|`build_read_scope_fixture`; `knowledge-read-scope-cases`| mcp/tests/read_scope_test_support.py:266-284; mcp/tests/evidence-lifecycle.toml:1261-1261; mcp/tests/evidence-lifecycle.toml:1297-1297; mcp/tests/evidence-lifecycle.toml:1267-1267 |
| The unit-lane row the read's selection module occupies. | "mcp/tests/test_knowledge_read_scope.py" | mcp/tests/test-evidence-lanes.toml:81-81 |
| The integration-lane row the read's boundary module occupies. | "mcp/tests/test_knowledge_read_boundaries.py" | mcp/tests/test-evidence-lanes.toml:169-169 |
| The integration-lane row the read's path module occupies, one entry below its boundary sibling. | "mcp/tests/test_knowledge_read_paths.py" | mcp/tests/test-evidence-lanes.toml:171-171 |
...
|**The fixture and the governed contract it is registered under.**|`build_diff_fixture`; `knowledge-diff-cases`| mcp/tests/diff_scope_test_support.py:189-233; mcp/tests/evidence-lifecycle.toml:1256-1277; mcp/tests/evidence-lifecycle.toml:1262-1262 |
| **The unit-lane row this leaf registered for the comparison's scope module.** | "mcp/tests/test_knowledge_diff_scope.py" | mcp/tests/test-evidence-lanes.toml:84-84 |
| **The integration-lane row this leaf registered for the comparison's boundary module.** | "mcp/tests/test_knowledge_diff_boundaries.py" | mcp/tests/test-evidence-lanes.toml:170-170 |
| **The catalog digest re-pin that a new test module obliges.** | `LIFECYCLE_CATALOG_SHA256` | mcp/tests/test_dependency_ownership_ast_helpers.py:43-65 |
...
| Finding | Anchor | Source |
| --- | --- | --- |
| **The byte-identity node: the shipped page and result compared against digests measured before this leaf existed.** | "test_the_shipped_seed_page_is_byte_identical_and_the_facet_page_is_its_own_policy" | mcp/tests/test_knowledge_facets.py:986-1035 |
...
|**The diff-comparison contract whose consumer list gained the run module (the consumer row is the third entry of that block).**|"contract:knowledge-diff-cases"| mcp/tests/evidence-lifecycle.toml:1283-1283 |
|**The read-scope contract whose consumer list gained the run module (the consumer row is the third entry of that block).**|"contract:knowledge-read-scope-cases"| mcp/tests/evidence-lifecycle.toml:1303-1303 |
...
| The two integration-lane rows the read's boundary and path modules occupy. | "mcp/tests/test_knowledge_read_boundaries.py"; "mcp/tests/test_knowledge_read_paths.py" | mcp/tests/test-evidence-lanes.toml:169-169; mcp/tests/test-evidence-lanes.toml:171-171; mcp/tests/test-evidence-lanes.toml:164-168 |
...
| **The two lane rows this leaf registered.** | "mcp/tests/test_knowledge_diff_scope.py"; "mcp/tests/test_knowledge_diff_boundaries.py" | mcp/tests/test-evidence-lanes.toml:170-170; mcp/tests/test-evidence-lanes.toml:81-167 |
| **The diff-comparison contract whose consumer list gained the run module (the consumer row is the third entry of that block).** | "contract:knowledge-diff-cases" | mcp/tests/evidence-lifecycle.toml:1283-1283 |
| **The read-scope contract whose consumer list gained the run module (the consumer row is the third entry of that block).** | "contract:knowledge-read-scope-cases" | mcp/tests/evidence-lifecycle.toml:1303-1303 |
...
| **The integration lane row this leaf opened, and the governed contract whose consumer list gained the boundary module.** | "contract:knowledge-read-scope-cases" | mcp/tests/evidence-lifecycle.toml:1303-1303; mcp/tests/test-evidence-lanes.toml:161-162 |
