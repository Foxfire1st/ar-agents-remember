...
## Selected Certification Fixture Boundaries

[`integration_certification_test_support.py`](integration_certification_test_support.py.md) creates actual integration journals and original physical code publications while distinguishing structural-only references. [`selected_lifecycle_test_support.py`](selected_lifecycle_test_support.py.md) obtains real doors/generations; its completion bookkeeping is not certification evidence. [`source_selection_test_support.py`](source_selection_test_support.py.md) builds synthetic compiler inputs and does not observe Git.

[`test_dagger_certification_suffix.py`](test_dagger_certification_suffix.py.md) checks frozen suffix and retained-byte boundaries with SDK doubles. [`test_closeout_memory_certification_reuse.py`](test_closeout_memory_certification_reuse.py.md) composes actual memory checker/coherence/closure owners, physical Gate-5 publication and selected CAS; its Node executor and pending finalizer remain injected. Read those boundaries before treating a focused test result as proof of installed production composition.
...
`test_rail_bindings.py` now requires a complete artifact map, stable report-relative references, exact bounded byte tails and detached-handle observations. `gate_certification_test_support.py` owns shared real Git/profile/lane/catalog fixture construction for the source-derived consumer population declared by the evidence-lifecycle catalog. Its code-gate payloads retain exact source-applicability decisions, planned zero starts and canonical census manifests produced from real temporary fixture files; real Git and publication readback do not turn injected rail execution into real Dagger evidence. `test_gate_certification_records.py` additionally corrupts real admission/certificate store objects to prove invalid semantic digest and wrong exact-address refusals. These tests protect the owner boundaries; real Dagger execution remains separate acceptance evidence. The closeout fixture composes real selected operation and worker state for its component tests, while its pending memory continuation and component-only writers remain explicit fixture boundaries. See [gate fixture composition](gate_certification_test_support.py.md) and [closeout fixture composition](closeout_fixture_test_support.py.md).
