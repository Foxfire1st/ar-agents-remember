...
The current retained population is 99 unit files, 2 public checks, 55 integration files, 14 architecture files and 13 provider files (183 files total). The closeout auto-carry change registered one new module, `test_sync_parked_candidate.py`, in the existing `unit-regression` lane and extended `test_source_lineage.py` with the `CloseoutSourceLineageHealTests` boundary cases; no case budget was raised. The L38 integration cards above describe admission/status projection and route-review transport. These counts and cards are preparation evidence, not Gate 5 acceptance.
