...
| Resolver-by-`worktree_name` tests build an external-memory skeleton (`_external_memory_skeleton`) and write real contracts (`_write_task_contract`) to prove `worktree_name` alone populates `contract_path`/`code_worktree`/`memory_worktree` (regression guard), an unknown `worktree_name` returns those fields `None` without raising, `task_name` wins over a competing `worktree_name`, and `find_worktree_contract` matches the recorded worktree group or returns `None`. | resolver worktree_name tests | [test_worktree_support.py](agents-remember/mcp/tests/test_worktree_support.py) |
...
- 2026-06-22T22:00+02:00 — Added four resolver-by-`worktree_name` tests plus `_external_memory_skeleton`/`_write_task_contract` helpers and a `worktree_group_for` import: happy-path regression guard (contract fields non-`None`), graceful-empty unknown name, `task_name`-over-`worktree_name` precedence, and the `find_worktree_contract` unit helper (#90).
- 2026-06-19T13:42: Rewrote `test_benchmark_provider_setup_seeds_grepai_from_source_provider` → `test_benchmark_provider_setup_is_hermetic_no_seed` (asserts no seed source for either provider) and dropped the seed-source assertions from `test_benchmark_provider_setup_uses_generated_settings_file`, matching hermetic-cold benchmark provider setup (task 260619).
