...
| Resolver tests cover arbitrary installed runtime roots by making a non-`ar-coordination` directory with `skills/`, `system/`, `tasks/`, and `memory-repos/`, then proving C-08 uses it as the coordination root. | L852-L875 | [test_worktree_support.py](agents-remember-md/mcp/tests/test_worktree_support.py) |
