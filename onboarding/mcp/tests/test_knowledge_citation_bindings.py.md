...
| The closed vocabulary agreeing with its facts in both directions. | `test_the_closed_vocabulary_and_its_facts_agree_in_both_directions` | mcp/tests/test_knowledge_citation_bindings.py:238-245 |
...
| **The lane row this module occupies, and the lane it sits in — the unit lane, because every case here is hermetic.** | "mcp/tests/test_knowledge_citation_bindings.py" | mcp/tests/test-evidence-lanes.toml:71-71 |
