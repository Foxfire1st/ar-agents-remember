...
| Finding | Anchor | Source |
| --- | --- | --- |
| Tracked, missing, staged and untracked caches do not prevent memory removal; real memory and code files remain protected. | `test_terminal_removal_discards_only_the_memory_cache` | mcp/tests/test_worktree_sync.py:615-657 |
| The real Git fixture and attributed official memory update. | `map_official_memory` | mcp/tests/test_worktree_sync.py:132-144 |
| Fast-forward and retained code conflict behavior. | `test_pure_fast_forward_sync_advances_both_sides_and_contract`; `test_code_merge_conflict_is_retained_and_can_continue` | mcp/tests/test_worktree_sync.py:494-514; mcp/tests/test_worktree_sync.py:516-551 |
| Cache-independent source admission, start, and candidate identity. | `test_sync_uses_source_refs_when_the_cache_is_stale_missing_or_malformed`; `test_start_and_memory_candidate_do_not_take_authority_from_the_cache` | mcp/tests/test_worktree_sync.py:553-579; mcp/tests/test_worktree_sync.py:581-613 |
| Native cache-only success, real conflict continuation, resumed staged-content validation, and the four knowledge-conflict scenarios this leaf's line added. | `test_memory_merge_settles_content_and_knowledge_conflicts_in_the_transaction`; `_assert_knowledge_conflict_scenarios`; `_assert_memory_content_conflict_scenarios` | mcp/tests/test_worktree_sync.py:698-719; mcp/tests/test_worktree_sync.py:721-761; mcp/tests/test_worktree_sync.py:763-844 |
| **The CYCLE-02 knowledge-dataset case: the sync completes, both sides survive, and the caller invokes no merge entry point.** | `_assert_knowledge_database_conflict_settles` | mcp/tests/test_worktree_sync.py:150-186 |
| **The CYCLE-02 residue case: the reversed orientation, whose arriving side removed the row the retained side cites, and the assertion that the call the response advertises actually moves the state.** | `_shape_delete_reference_reversed`; `_assert_unretractable_delete_reference_advertises_its_real_route` | mcp/tests/test_worktree_sync.py:232-247; mcp/tests/test_worktree_sync.py:402-455 |
...
| Nonregular journal quarantine preserves the outside target. | `test_nonregular_journal_is_renamed_without_following_and_quarantined` | mcp/tests/test_worktree_sync.py:881-900 |
...
## Update History
- 2026-09-20T07:30+02:00 — 260915-KS-L42 curator (uncommitted CYCLE-02 repair change set on `ar/260915-ks-l42-ar`, code base `74c6c693`): **the module gained the case for the orientation round 2 left open, and this card's inventory of what the module drives is corrected rather than annotated.** Added `_shape_delete_reference_reversed` (the arriving side removes the anchor the retained side's realization claim cites, so the arriving delta carries only the DELETE) and `_assert_unretractable_delete_reference_advertises_its_real_route`, which asserts the corrected public surface for that orientation — the engine's diagnosis unchanged, `precondition == "no_arriving_insertion"`, `decisions` empty, `nextOperation=continue_sync_resolution` with no `knowledge_resolution` in `nextArgs`, a summary that names the conflict and says to restore the removed row, `cancelArgs` still carried — and then drives exactly what the response advertised and requires the state to **change** (`sync-resolution-incomplete`) with HEAD and all three input datasets byte-identical. Both helpers run from the existing retained-conflict case, so no collected case was added to a lane at its exact ceiling. Seven citation ranges were re-cited to their constructs' own declaration extents in the working tree: the four `WorktreeSyncTests` cases the new module-level helpers pushed down (`test_pure_fast_forward_sync_advances_both_sides_and_contract`, `test_code_merge_conflict_is_retained_and_can_continue`, `test_sync_uses_source_refs_when_the_cache_is_stale_missing_or_malformed`, `test_start_and_memory_candidate_do_not_take_authority_from_the_cache`), the three case-running helpers (`test_memory_merge_settles_content_and_knowledge_conflicts_in_the_transaction`, `_assert_knowledge_conflict_scenarios`, `_assert_memory_content_conflict_scenarios`), `_assert_knowledge_database_conflict_settles`, `test_terminal_removal_discards_only_the_memory_cache` and `test_nonregular_journal_is_renamed_without_following_and_quarantined`. One row and one invariant were added for the new case; no claim was weakened and no anchor dropped. **One item is left OPEN, not settled:** `cancelArgs` returns `sync-operation-refused` / `SyncGitProofError` in both orientations — including the INSERTED-row orientation round 2 verified as working — so the cancel half of the manual continuation is unproven in this fixture and is named for the round-3 reviewer. Verification metadata is advanced to this leaf's own base `74c6c693` with the uncommitted working candidate beside it; closeout owns the committed stamp.
