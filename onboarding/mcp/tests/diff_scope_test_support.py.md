...
| The unit-lane row that keeps the scope module in the certifying collection path. | "mcp/tests/test_knowledge_diff_scope.py" | mcp/tests/test-evidence-lanes.toml:106-106 |
| The integration-lane row that keeps the boundary module in the certifying collection path. | "mcp/tests/test_knowledge_diff_boundaries.py" | mcp/tests/test-evidence-lanes.toml:207-207 |
...
## Update History
- 2026-09-20T02:25+02:00 — 260915-KS-L31 curator (uncommitted CYCLE-02 change set on `ar/260915-ks-l31-ar`, code base `7dcec036`): **cleared all 10 enforced `citation_anchor_absent_from_range` rows this card carried by hand-reading `mcp/tests/evidence-lifecycle.toml` on the merged line, with no claim re-worded and no anchor changed.** Every one of the ten is the same displacement: the sync onto the master line inserted two lines above the `mcp/tests/diff_scope_test_support.py` `[[artifact]]` block, so each construct the rows cite moved +2 and every range that was correct on the pre-sync line is now two lines short. The artifact row's own fields are at `introduced_by = "260915-KS-L8"` on **1393** (cited 1391) and `replacement_contract = "contract:knowledge-diff-cases"` on **1396** (cited 1394) — repaired in the section row **and in both byte-identical duplicates** (the repo-internal block's second copy and the cross-repo row), five rows in total. The `read_scope_test_support.py` artifact's `replacement_contract = "contract:knowledge-read-scope-cases"` is on **1418** (cited 1416), beside `"contract:knowledge-diff-cases"` on **1396** (cited 1394), repaired in both copies of the dual-contract row. Each range now sits on the declaration line of the anchor it is paired with, so no range is carried merely because the pooled membership test would tolerate it. Anchors, claim wording and every other cell are unchanged; no verification stamp advanced (the candidate is uncommitted and the governed closeout owns the real code and memory commits), and the `Generated citation repair` bullets below are left standing because none of them is what kept these rows open — each was a range the checker refused, and this pass is the reading that cleared it. No commits.
...
- 2026-09-20T00:16:01+00:00: Generated citation repair: "contract:knowledge-diff-cases" repointed to mcp/tests/evidence-lifecycle.toml:1396-1396. No content impact: mechanical anchor-range projection bound to citation source snapshot b8fe5b3589f1357e836aaad1587e69ed38bbda0d58221eaa2150e96eb0561e93; claim bytes unchanged; generated by ccr-r10@v1.
- 2026-09-20T00:16:01+00:00: Generated citation repair: "contract:knowledge-read-scope-cases" repointed to mcp/tests/evidence-lifecycle.toml:1418-1418. No content impact: mechanical anchor-range projection bound to citation source snapshot b8fe5b3589f1357e836aaad1587e69ed38bbda0d58221eaa2150e96eb0561e93; claim bytes unchanged; generated by ccr-r10@v1.
- 2026-09-20T01:04:20+02:00 — 260915-KS citation residue clearance (uncommitted change set on memory base `66b2ae8adebea11bc2300d2d51822f321a128657`): cleared the 7 enforced citation rows this card carried (citation_anchor_absent_from_range ×7). The earlier mechanical projection had left three cells still naming fields that had shifted inside the `mcp/tests/diff_scope_test_support.py` artifact row: hand-reading `mcp/tests/evidence-lifecycle.toml` showed `introduced_by = "260915-KS-L8"` at line 1391 and `replacement_contract = "contract:knowledge-diff-cases"` at 1394, so the `introduced_by` cell moved from `:1388-1388` to `:1391-1391` (in the section row) and, in the two identical duplicate rows, the same `:1388-1388` cell moved to `:1391-1391` beside the already-correct `:1394-1394`. The remaining four rows already read `:1394-1394` (contract) and `:1416-1416` (read-scope contract) and were verified current, as were the `id = "…"` cells at `:60-62` / `:65-67`. Claim wording, anchors and every other range are unchanged; no verification stamp was advanced.
- 2026-09-20T00:31+02:00 — 260915-KS-L30 curator (uncommitted change set on `ar/260915-ks-l30-ar`, base `7dcec036094768c5f50e571fb45e59a27ae78efc`): **mechanical citation-range projection** against this leaf's candidate. The checklist reported 6 row(s) whose cited range no longer holds its anchor although the construct is present in the cited file; each range was widened to the lines that carry it — `contract:knowledge-diff-cases`; `contract:knowledge-read-scope-cases`. No claim wording, anchor or citation was added, removed or re-worded, and no range was deleted: the new range is the checklist's own resolved extent for that anchor on this candidate. No verification stamp was advanced.
