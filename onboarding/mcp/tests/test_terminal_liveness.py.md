...
- 2026-09-15T13:36+02:00 — 260831-LOCR-L27 curator (uncommitted change set on `ar/260831-locr-l27`,
  base `b368b661`): **No content impact:** this module is byte-unchanged by the leaf
  (`git hash-object` = `7cb3828d8380a0ada1107afe9ac31f566fdafecc` = `HEAD:mcp/tests/test_terminal_liveness.py`,
  574 lines), so every case, range and boundary recorded above still holds exactly as written and no
  row was added, removed or moved. The leaf's new pane-authority proof was deliberately **split out**
  instead of extending this module: an in-place extension reached 1094 lines, inside the
  `system/coding-guidelines.md` 900-1200 "soft limit exceeded / do not add new feature logic without
  extracting" band, so
  [test_terminal_liveness_pane_authority.py](test_terminal_liveness_pane_authority.py.md) is a separate
  registered module and this one keeps its role as the sweeper-boundary suite. The new module
  **imports** this file's fixtures (`_Clock`, `_FakeHost`, `_entry`, `_snapshot`, `_ready_snapshot`)
  rather than rebuilding them, so the two cards are coupled by that import and neither should be
  merged into the other. Verification metadata stays pinned until closeout stamps the leaf code commit.
