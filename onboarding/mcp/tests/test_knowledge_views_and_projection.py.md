...
`_build_mounted_family_fixture` (`:868-1020`, the store and its fixture record),
`_assert_family_traversal_from_one_path` (`:1435`), `_assert_the_path_seed_selects_in_every_view`
(`:1525`) and `_assert_the_context_is_constructible` (`:1585`), composed by
`_assert_the_family_read_returns_stored_membership_and_the_path_seed` (`:1617`), which
`test_the_read_family_refuses_a_sixth_view_before_it_touches_a_dataset` calls at its own end
(`:1384`); `test_the_read_family_refuses_a_continuation_minted_for_another_walk` calls the
constructible-context helper beside its refusal (`:1431-1432`). Reverting only
