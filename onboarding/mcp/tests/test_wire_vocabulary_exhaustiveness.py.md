...
Provides vocabulary-boundary helpers and constants only; no test functions remain. It creates a minimal contract, validates a field through WorktreeSummary, and parses source modules for consumers. The large historical docstring describes removed exhaustive producer scans and does not establish current protection. Runtime tolerant-reader/strict-writer tests live in the retained boundary companion.
...
| Finding | Anchor | Source |
| --- | --- | --- |
| Shared helper worktree_summary_accepts | `worktree_summary_accepts` | mcp/tests/test_wire_vocabulary_exhaustiveness.py:147-148 |
| Shared helper _contract | `_contract` | mcp/tests/test_wire_vocabulary_exhaustiveness.py:159-180 |
...
- 2026-09-14T19:00+02:00 — 260913-LCA-L12 curator (residue citation pass): re-derived the source
  ...
  --fix --document`). 5 further claim(s) were declined because the solution they name no longer
...
- 2026-08-01T08:35+02:00 — 260731-EFA-L4 curator: created for the new leaf-central wire-vocabulary
  ...
  cit:([`VOCABULARY_DECLARING_MODULE`], mcp/tests/test_wire_vocabulary_exhaustiveness.py:144-144), applied by both scan branches:
