...
| The module declares the integration marker at module level and pins the renderer version its destination profile and rendered outputs both carry. | "pytestmark = pytest.mark.integration" | mcp/tests/test_knowledge_projection_vault_safety.py:46-48; mcp/tests/test-evidence-lanes.toml:194-269 |
...
| Three local helpers name the acceptance profile, build one rendered output over the pinned renderer version, and read the destination manifest. | `_manifest` | mcp/tests/test_knowledge_projection_vault_safety.py:58-80 |
...
| A destination-wide byte snapshot is what turns "the prior generation is intact" into an exact comparison rather than a listing. | `_tree_bytes` | mcp/tests/test_knowledge_projection_vault_safety.py:249-256 |
