...
| An authored parent yields an acyclic hierarchy; an unauthored parent is `missing_expected_row` — one rule with both outcomes, and two cases until they were merged. | `test_a_childs_parent_must_be_an_authored_route` | mcp/tests/test_knowledge_routes.py:100-122 |
