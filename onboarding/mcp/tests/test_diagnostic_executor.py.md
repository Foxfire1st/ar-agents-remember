...
| --- | --- | --- |
| A diagnostic may run only after the exact candidate's R12 Gates 1-3 are green and every run binds the trusted frozen authority. | `DiagnosticExecutorAdmissionTests.test_admission_runs_only_after_gates_1_3_are_green`; `DiagnosticExecutorRunTests.test_one_pass_replication_binds_the_frozen_authority` | mcp/tests/test_diagnostic_executor.py:455-473; mcp/tests/test_diagnostic_executor.py:563-606 |
...
| Asserts R16 diagnostic telemetry envelope shape on started/terminal events. | `RecordingRunner`; `compile_diagnostic_started` | mcp/tests/test_diagnostic_executor.py:321-379; mcp/src/agents_remember/certification/telemetry/adapters.py:1-200 |
