...
| **The pre-existing sibling node's `detail` assertion, which round 3 added (the fourth worktree edit).** | "the unavailable tree is the producer that answers here, and its detail says which fact it is" | mcp/tests/test_knowledge_read_paths.py:521-526 |
| The lane row this module occupies. | "mcp/tests/test_knowledge_read_paths.py" | mcp/tests/test-evidence-lanes.toml:160-160 |
