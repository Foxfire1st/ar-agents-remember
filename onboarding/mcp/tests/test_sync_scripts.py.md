...
`.github/workflows/` (`quality-checks.yml`, `publish-mcp-to-pypi.yml`) mentions
`sync-skills.py`, `sync-runtime.py` or
...
| The only pre-L4 drift check: the locally installed hook gate calling both scripts with `--check` (installed by `setup-hooks.sh` L36, which sets `core.hooksPath`). | "scripts/sync-skills.py", "scripts/sync-runtime.py" | .githooks/_gate.sh:105-106 |
...
- 2026-08-28T15:52:15+02:00 — Rebased the skill/runtime sync-gate citation after the MCP-local
  interpreter selection repair shifted `_gate.sh`; the test contracts remain unchanged.
