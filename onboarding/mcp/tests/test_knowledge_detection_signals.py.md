...
| The generation builder this module measures against. | `create_schema_statements` | mcp/src/agents_remember/memory/knowledge/schema_generations.py:173-179 |
| **The registry that now holds five generations, so "the newest supported generation" is generation 5 and generation 4 is the detection rule's own pinned generation rather than the tip.** | `GENERATIONS` | mcp/src/agents_remember/memory/knowledge/schema_generations.py:365-372 |
