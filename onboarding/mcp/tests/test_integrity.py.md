...
legacy venv files. The status-success regression uses a realistic ready watcher
payload and expects the provider packet state to come from current provider
state.
...
- Provider status must proceed to watcher status when only legacy host venv
  files exist, and its final state should reflect current watcher status.
...
| The third test verifies provider status proceeds instead of returning `runnerIntegrityFailed` when only legacy CGC venv files exist, then reports ready current state. | L50-L90 | [test_integrity.py](agents-remember-md/mcp/tests/test_integrity.py) |
