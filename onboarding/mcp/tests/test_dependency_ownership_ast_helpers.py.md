...
`73cdd2183e153af752773e8a4c6076e5e2eb7053d9e0ce5fa6538a69c907cefe`, which `sha256sum
mcp/tests/evidence-lifecycle.toml` reproduces on this worktree** — re-pinned by `260915-KS-L30`, whose
one appended consumer row is the whole byte change, and correct only at this candidate's tip. The value
...
carries the fifteenth contract and sixty-fifth artifact — the running ordinal is ambiguous and this card
names values by leaf rather than minting another meaning**. The value at this
candidate is **15 contracts / 65 artifacts** and
**`73cdd2183e153af752773e8a4c6076e5e2eb7053d9e0ce5fa6538a69c907cefe`**, recomputed directly from the
...
carries", while the line now carries this leaf's `73cdd218…` — the constants, not the prose, are the
authority, and the text is the owning builder's to correct. The reading a reader should
take is the constants' own — 15 / 65 / `73cdd218…` — and the remaining residue is recorded here for the
...
| Finding | Anchor | Source |
| --- | --- | --- |
| Repository inputs reach their supported consumers. | `test_repository_inputs_reach_their_supported_consumers` | mcp/tests/test_dependency_ownership_ast_helpers.py:290-310 |
| **The evidence catalog's pinned shape and bytes, and the deliberate re-pin this leaf made in the same change as its catalog rows — whose value at this candidate is 15 contracts / 65 artifacts and `73cdd218…`, re-pinned by `260915-KS-L30` from L28's `6ec7eb0d…`.** | `LIFECYCLE_CONTRACT_COUNT`; `LIFECYCLE_ARTIFACT_COUNT`; `LIFECYCLE_CATALOG_SHA256` | mcp/tests/test_dependency_ownership_ast_helpers.py:43-46; mcp/tests/test_dependency_ownership_ast_helpers.py:44-46 |
| **The check that makes the pin real: the file's bytes recomputed and both block kinds counted before either is compared.** | `_assert_the_catalog_kept_its_bytes_and_identities` | mcp/tests/test_dependency_ownership_ast_helpers.py:380-397 |
...
| The catalog pin constants, and the re-pin narrative the docstring carries beside them — **stale on this candidate: its opening paragraph still presents L28's `6ec7eb0d…` as "the value this line carries" while the line now carries this leaf's `73cdd218…`.** | `LIFECYCLE_SCHEMA`; `LIFECYCLE_CONTRACT_COUNT`; `LIFECYCLE_ARTIFACT_COUNT`; `LIFECYCLE_CATALOG_SHA256` | mcp/tests/test_dependency_ownership_ast_helpers.py:43-43; mcp/tests/test_dependency_ownership_ast_helpers.py:44-44; mcp/tests/test_dependency_ownership_ast_helpers.py:45-45; mcp/tests/test_dependency_ownership_ast_helpers.py:46-46; mcp/tests/test_dependency_ownership_ast_helpers.py:47-109 |
| The three consumer entries the `260915-CAPS-L17` re-pin added to three governed-artifact rows, the entire reason the digest moved at that re-pin. | "mcp/tests/fixtures/repository_profiles/node/package-lock.json"; "mcp/tests/eve_capsule_test_support.py"; "mcp/tests/eve_adapter_test_support.py" | mcp/tests/evidence-lifecycle.toml:660-660; mcp/tests/evidence-lifecycle.toml:1453-1453; mcp/tests/evidence-lifecycle.toml:1475-1475 |
...
source's own text) → `73cdd218…` (L30, this candidate), each correct only at its own tip.
