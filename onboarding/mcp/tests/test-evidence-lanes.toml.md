...
the new gate-certificate forcing suite in the `unit-regression` lane. L10 registered the CCR-R10
deterministic anchor-range projection forcing suite (`mcp/tests/test_citation_deterministic_projection.py`)
in the `integration` lane in the same change that created it.
...
L10 added `mcp/tests/test_citation_deterministic_projection.py` to the `integration` lane: the
suite drives the real citation fixer and source-index snapshot over temporary on-disk repositories,
so its process-backed integration evidence class is deliberate rather than inferred.
...
| The CCR-R10 deterministic anchor-range projection suite is explicitly integration evidence. | "mcp/tests/test_citation_deterministic_projection.py" | mcp/tests/test-evidence-lanes.toml:223-223 |
