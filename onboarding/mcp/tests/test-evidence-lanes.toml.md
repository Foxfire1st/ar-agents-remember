...
260831-CCR-L23 added `mcp/tests/test_serving_requirements.py` to the `integration` lane:
the requirement-route suite drives a real FastAPI app over a temporary coordination tree with
process-backed Git operations, so its evidence class is deliberate rather than inferred.
...
| The L23 task-local requirement-route suite is explicit integration evidence. | "mcp/tests/test_serving_requirements.py" | mcp/tests/test-evidence-lanes.toml:419-419 |
