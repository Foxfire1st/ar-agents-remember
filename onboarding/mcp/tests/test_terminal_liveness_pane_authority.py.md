...
| The canonical adapter snapshot contract whose `ControlState` includes the `disconnected`/`failed` values the readiness case drives. | `AdapterSnapshot`; `ControlState` | mcp/src/agents_remember/models/conversations/control_wire.py:19 |
...
| The lane registration the fail-closed manifest requires for this module. | "mcp/tests/test_terminal_liveness_pane_authority.py" | mcp/tests/test-evidence-lanes.toml:187-187 |
