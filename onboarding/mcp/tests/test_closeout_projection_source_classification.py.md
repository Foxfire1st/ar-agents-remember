...
| The integration lane row the fail-closed manifest requires. | "mcp/tests/test_closeout_projection_source_classification.py" | mcp/tests/test-evidence-lanes.toml:143-143 |
...
| Finding | Anchor | Source |
| --- | --- | --- |
| The fixture's repositories are real temporary Git repositories created per case, not mocks. | `QueueFixture` | mcp/tests/test_closeout_queue.py:186-727 |
