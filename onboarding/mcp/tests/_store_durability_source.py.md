...
| The harness preserves its public helper surface by listing both imported names in `__all__`. | "    \"extract_base_commit_tree\","; "    \"run_against_source\"," | mcp/tests/_store_durability.py:85-91 |
| The executable harness retains the source-root guard and owns `main`; this sibling only prepares and launches that entry point. | `_require_source_root`; `main` | mcp/tests/_store_durability.py:1096-1103; mcp/tests/_store_durability.py:1107-1120 |
| The control-plane contract imports both public helpers to prove the defect against the archived base tree. | `extract_base_commit_tree`; `run_against_source` | mcp/tests/test_controlplane_store_durability.py:42-52 |
| The provider contract imports the same helpers so both store families use one pinned-source mechanism. | `extract_base_commit_tree`; `run_against_source` | mcp/tests/test_provider_store_durability.py:72-79 |
