...
| Finding | Anchor | Source |
| --- | --- | --- |
| The wrapper calls both rails after pytest and repeats them only after an inconclusive conservative delta explicitly requires a fresh full rerun. | `run_coverage_rails`; `complete_coverage_rails` | mcp/test_support/agents_remember_test_support/code_quality/check.py:253-295; mcp/test_support/agents_remember_test_support/code_quality/check.py:368-392 |
| CRAP calculation uses Coverage.py branch units and Radon complexity. | `calculate_scores` | mcp/test_support/agents_remember_test_support/code_quality/crap_calculator.py:294-305 |
| Diff measurement intersects coverage units with the resolved Git diff. | `measure` | mcp/test_support/agents_remember_test_support/code_quality/diff_coverage.py:289-317 |
