...
| 6 | Worktree contract | `tasks/<repo>/<task>/contract.md` (YAML front matter `ar-worktree-contract/v1`) | `worktree_contract.py`; mutated at each lifecycle transition | Per-task kanban: started → approved → closed out → integrated → cleaned; approval audit |
