...
D4 — Runtime stores are not the survival layer (hard rule). Information that must survive belongs in task documentation or a database — never in a runtime store. Runtime stores are working surfaces (notification, scheduling, dedup): always bounded, always evictable, never the record. When boundedness and row preservation conflict, the row drops — pending rows included — and that is correct behavior, not data loss; observability of a drop (a counted/surfaced fact) is welcome where cheap but never justifies exceeding the bound. This priority reigns above every retention, redelivery, or terminal-honesty guarantee built on the store. (Developer ruling 2026-08-08, stated as always-operative global doctrine.)
